...
public sealed partial class AlsInstructionExtractionPolicy : IInstructionExtractionPolicy, IInstructionDocumentProfile, IInstructionFieldRoles
{
    ...
    static readonly InstructionFieldEngine.FieldDefinition[] Definitions = [new("Claimant name", ["Client name"], PartyRole: "claimant"), new("Vehicle owner", ["Vehicle owner"], IsRequired: false, PartyRole: "owner"), new("Claim reference", ["Our Reference"], PartyRole: "principal", ReferenceRole: "principal"), new("Vehicle registration", ["Client registration"], IsValidTyped: InstructionFieldEngine.IsUkRegistration, CanonicalValue: InstructionFieldEngine.NormalizeRegistration, PartyRole: "claimant"), new("Vehicle make", ["Client vehicle make"], PartyRole: "claimant"), new("Vehicle model", ["Client vehicle model"], IsRequired: false, PartyRole: "claimant"), new("Incident date", ["Accident Date"], IsValidTyped: v => InstructionFieldEngine.ParseDate(v) is not null, CanonicalValue: InstructionFieldEngine.CanonicalDate, PartyRole: "claimant"), new("Inspection address", ["Vehicle location"], IsRequired: false, PartyRole: "inspection-location"), new("Accident circumstances", ["Accident Circumstances"], IsRequired: false, PartyRole: "claimant"), new("VAT status", ["VAT Registered"], IsRequired: false, PartyRole: "claimant"), new("Repairer", ["Repairer"], IsRequired: false, PartyRole: "repairer"), new("Claimant address", ["Client address"], IsRequired: false, PartyRole: "claimant"), new("Vehicle class", ["Vehicle Category"], IsRequired: false, PartyRole: "vehicle-class"), new("Inspection date", ["Completed inspection date"], IsRequired: false, IsValidTyped: v => InstructionFieldEngine.ParseDate(v) is not null, CanonicalValue: InstructionFieldEngine.CanonicalDate, PartyRole: "inspection")]; static readonly InstructionFieldEngine.LabelRegexCache Cache = new(Definitions);
...
public InstructionExtractionResult Extract(IntakeSourceReadResult r, InstructionExtractionTiming timing, EstablishedPrincipalContext p) { if (r.Status != IntakeSourceReadStatus.Readable || r.IsIncomplete) throw new ArgumentException("ALS extraction requires complete readable content.", nameof(r)); if (p.PrincipalCode != SupportedPrincipalCode) throw new ArgumentException("The established principal is not ALS.", nameof(p)); var (f, m, e) = InstructionFieldEngine.ExtractFields(InstructionFields(r.Content).ToArray(), Definitions, Cache, timing); var v = f.ToDictionary(x => x.Name, x => x.SuggestedValue, StringComparer.Ordinal); var d = new InstructionDraft("ALS", InstructionFieldEngine.TypedString(v["Claimant name"], 300), InstructionFieldEngine.TypedString(v["Claim reference"], 100), InstructionFieldEngine.NormalizeRegistration(v["Vehicle registration"]), InstructionFieldEngine.TypedString(v["Vehicle make"], 100), InstructionFieldEngine.TypedString(v["Vehicle model"], 100), null, InstructionFieldEngine.TypedString(v["Accident circumstances"], 2000), InstructionFieldEngine.ParseDate(v["Incident date"]), InstructionFieldEngine.TypedString(v["Inspection address"], 1000), InstructionFieldEngine.ParseDate(v["Inspection date"]), null, InstructionFieldEngine.TypedString(v["VAT status"], 100), null, null); return new(InstructionPolicyApplicability.Applicable, [.. e, new(IntakeEvidenceSource.Sender, IntakeEvidenceStrength.Strong, IntakeEvidenceFinding.SupportsPrincipal, "established-principal", $"Principal ALS was established by {p.PolicyKey} v{p.PolicyVersion}.")], f, d, m, Key, Version); }
    static IEnumerable<IntakeContentFragment> Fields(IntakeContentFragment f, bool includeVehicle) { var t = f.Text.Replace("\r\n", "\n", StringComparison.Ordinal).Replace('\r', '\n'); var dear = Dear().Match(t); var sign = Sign().Match(t); if (!dear.Success || !sign.Success || sign.Index <= dear.Index) yield break; var h = t[..dear.Index]; var b = t[dear.Index..sign.Index]; foreach (Match x in Ref().Matches(h)) yield return L(f, "Our Reference", x.Groups["v"].Value); foreach (Match x in Accident().Matches(b)) yield return L(f, "Accident Date", x.Groups["v"].Value); foreach (Match x in Circ().Matches(b)) yield return L(f, "Accident Circumstances", x.Groups["v"].Value); foreach (Match x in Party().Matches(b)) { yield return L(f, "Client name", x.Groups["client"].Value); yield return L(f, "Client address", x.Groups["address"].Value); } foreach (Match x in Owner().Matches(b)) yield return L(f, "Vehicle owner", x.Groups["owner"].Value); foreach (Match x in Vat().Matches(b)) yield return L(f, "VAT Registered", x.Groups["v"].Value); if (includeVehicle) foreach (var vehicle in VehicleFields(f, b)) yield return vehicle; foreach (Match x in Location().Matches(b)) if (x.Groups["v"].Value.Trim(" .".ToCharArray()).Length > 0) yield return L(f, "Vehicle location", x.Groups["v"].Value); foreach (Match x in Repairer().Matches(b)) yield return L(f, "Repairer", x.Groups["v"].Value); }

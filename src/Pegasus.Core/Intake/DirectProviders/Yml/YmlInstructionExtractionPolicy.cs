...
public InstructionExtractionResult Extract(IntakeSourceReadResult readResult, InstructionExtractionTiming timing, EstablishedPrincipalContext principalContext)
    {
        ...
        var (fields, missing, extracted) = InstructionFieldEngine.ExtractFields(readResult.Content.SelectMany(Fields).ToArray(), Definitions, Cache, timing);
    ...
    private static IEnumerable<IntakeContentFragment> Fields(IntakeContentFragment f)
    {
        ...
        foreach(Match m in RefRegex().Matches(h)) yield return L(f,"Our Ref",m.Groups["value"].Value);
    ...
    [GeneratedRegex(@"(?im)^\s*Our\s+Ref\s*:[ \t]*(?<value>[^\r\n]+)",RegexOptions.CultureInvariant,100)]private static partial Regex RefRegex();

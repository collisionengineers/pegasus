    ...
    IRevokeRequestUploadLink revokeRequestUploadLink) : PageModel
{
    ...
    private readonly IRevokeBoxFileRequest revokeBoxFileRequest =
    ...
    public async Task<IActionResult> OnPostRevokeBoxAsync(
        Guid requestId,
        Guid caseId,
        ...
        CancellationToken cancellationToken)
    {
            ...
            !TryGetLease(caseId, out var leaseToken))
        {
            ...
            StatusMessage = "The Box file-request revocation requires current edit mode. Refresh and try again.";
            return RedirectToPage();
        }

        try
        {
            await revokeBoxFileRequest.ExecuteAsync(
                new(
                    caseId,
                    requestId,
                    actor,
                ...
                cancellationToken);
            ClearLease();
            StatusMessage = "The Box file request was revoked.";
        }
        catch (StaffAuthorizationException)
        {
            return Forbid();
        ...
        {
            PreserveReason(requestId, reason);
            StatusMessage = "The Box file-request revocation was invalid. Refresh and try again.";
        }
        catch (InvalidOperationException)
        {
            PreserveReason(requestId, reason);
            StatusMessage = "The Box file request or edit mode changed before revocation. Refresh and try again.";
        }
        catch (DbUpdateConcurrencyException)
        {
            PreserveReason(requestId, reason);
            StatusMessage = "The Box file request changed before revocation. Refresh and try again.";
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            PreserveReason(requestId, reason);
            StatusMessage = "The Pegasus upload-link revocation requires current edit mode. Refresh and try again.";
            return RedirectToPage();
        }
        ...
        try
        {
            await revokeRequestUploadLink.ExecuteAsync(
                ...
                cancellationToken);
            ClearLease();
            StatusMessage = "The Pegasus upload link was revoked.";
        }
        catch (StaffAuthorizationException)
        {
            return Forbid();
        ...
        {
            PreserveReason(requestId, reason);
            StatusMessage = "The Pegasus upload-link revocation was invalid. Refresh and try again.";
        }
        catch (InvalidOperationException)
        {
            PreserveReason(requestId, reason);
            StatusMessage = "The Pegasus upload link or edit mode changed before revocation. Refresh and try again.";
        }
        catch (DbUpdateConcurrencyException)
        {
            PreserveReason(requestId, reason);
            StatusMessage = "The Pegasus upload link changed before revocation. Refresh and try again.";
        }
    ...
    public static string KindLabel(RequestOperationKind kind) => kind switch
    {
        ...
        RequestOperationKind.PegasusUploadLink => "Pegasus upload link",
        RequestOperationKind.ExternalWork => "External work",
        _ => throw new InvalidOperationException(
            $"Unknown request operation kind value '{(int)kind}'.")
    };
    ...
    public static string StateLabel(RequestOperationState state) => state switch
    {
        ...
        _ => throw new InvalidOperationException(
            $"Unknown request operation state value '{(int)state}'.")
    };

    public static string LeaseLabel(RequestCaseEditLeaseState state) => state switch
    {
        RequestCaseEditLeaseState.Available => "Edit mode available",
        RequestCaseEditLeaseState.Active => "Edit mode active",
        RequestCaseEditLeaseState.Unknown => "Edit mode unavailable",
        _ => throw new InvalidOperationException(

// The five Upload flow alternatives from the first pass of this round
// (24 September 2026), now rendered inside the rebuilt live shell. The
// content functions are the first pass's, unchanged; only the frame around
// them moved from the v28 capture (which still carried the working-set
// strip) to the shell _Layout.cshtml renders today.
...
export const uploadStates = ['select', 'chosen', 'storing', 'processing', 'decision', 'registered', 'mixed', 'single', 'failed', 'discarded', 'no-match', 'multiple', 'attached'];
const fileNames = [
...
function fileRows(state, mode = 'rows') {
  ...
  return fileNames.slice(0, show).map((name, i) => {
    ...
    const iconMarkup = mode === 'gallery' ? `<span class="tile-thumb" aria-hidden="true">${glyph}<span>JPEG</span></span>` : `<span class="file-glyph" aria-hidden="true">${glyph}</span>`;
    const detail = `<small>${(0.23 + i * 0.01).toFixed(2)} MB</small>`;
    return `<li class="file-row" data-file-row>${iconMarkup}<span class="file-name" title="${esc(name)}"><strong>${esc(name)}</strong>${detail}</span>${statusFor(state, i)}</li>`;
  }).join('');
}
...
export function uploadMain(state, variant) {
...
.mock-dialog-backdrop{position:fixed;inset:0;z-index:1200;background:#0008;display:grid;place-items:center}.mock-dialog-backdrop .dialog{width:min(440px,calc(100vw - 30px));padding:20px;border:1px solid var(--line);border-radius:4px}
.mock-dialog-backdrop .dialog h2{font-size:18px;margin:0 0 12px}.mock-dialog-backdrop .dialog p{color:var(--muted);line-height:1.6;font-size:13px}
.mock-dialog-backdrop .dialog .action-row{display:flex;justify-content:flex-end;gap:8px;margin-top:24px}
...
// The interactions the first pass had: propose, find, discard, confirm.
...
(function(){
  ...
  surface.addEventListener('input',e=>e.target.setCustomValidity?.(''));
  function showDialog(title,message,action,next){const previous=document.activeElement;const backdrop=document.createElement('div');backdrop.className='mock-dialog-backdrop';backdrop.innerHTML='<div class="dialog" role="dialog" aria-modal="true" aria-labelledby="mock-dialog-title"><h2 id="mock-dialog-title">'+esc(title)+'</h2><p>'+esc(message)+'</p><div class="action-row"><button class="btn '+(next==='discarded'?'btn--danger':'btn--primary')+'" data-accept>'+action+'</button><button class="btn" data-close>Cancel</button></div></div>';document.body.append(backdrop);const close=()=>{backdrop.remove();previous?.focus()};backdrop.querySelector('[data-accept]').onclick=()=>{const q=new URLSearchParams(location.search);q.set('state',next);if(title.includes('QDOS25984'))q.set('case','QDOS25984');else q.delete('case');location.search=q.toString()};backdrop.querySelector('[data-close]').onclick=close;backdrop.addEventListener('keydown',x=>{if(x.key==='Escape')close();if(x.key==='Tab'){x.preventDefault();const buttons=backdrop.querySelectorAll('button');(document.activeElement===buttons[0]?buttons[1]:buttons[0]).focus()}});backdrop.querySelector('[data-close]').focus()}
  const params=new URLSearchParams(location.search);
  if(state==='attached'&&params.get('case')==='QDOS25984'){const panel=surface.querySelector('.decision-panel');if(panel)panel.innerHTML='<h2>Added to Case QDOS25984</h2><p>BH17RZV · A. Taylor · With Engineer</p><p>11 files</p>'+link('New upload','select')}
  window.mockup={variant:document.body.dataset.variant,state,files:surface.querySelectorAll('[data-file-row]').length};
})();
`;

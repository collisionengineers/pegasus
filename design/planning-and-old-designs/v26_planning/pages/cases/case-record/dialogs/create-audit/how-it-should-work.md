...
- An **Inspection + Audit** Case offers **Create audit** in the Actions menu (decided 13 September: the record bar keeps only Edit, Save and Cancel; Create audit and Send to EVA are menu items) once a report has been generated on it (sent is not required). It is absent before that, on Inspection and Audit Cases, once the Audit exists, and on a Case recorded Created in error or archived.
...
- Its reference is derived from the Inspection's recorded outcome: **Total loss → `ap.{Case/PO}`**; **Repairable, Cash in lieu and Contract repair → `a.{Case/PO}`** (the last two are repairable-basis outcomes, so the dialog never asks; decided 13 September).
...
Compact. Facts: Original case, Outcome, Audit reference. One primary, **Create audit**. No reason is required; the action is its own record.
...
- The Audit Case inherits the original's engineer.
- Cash in lieu and Contract repair derive `a.`; the dialog never asks.
- Create audit is offered once a report has been generated; sending it is not required.

...
| Operator, 13 Sep (v24 notes) | Overview shows the Principal record's Notes on every Case and the Claim source record's, read-only and locked, absent when empty; beside each an editable notes field for this Case. They sit in a Notes band under the three columns (record notes left, this-Case notes right) so the Principal column stays level with Case and Claimant | Overview › Notes band; [`../../administration/contacts/how-it-should-work.md`](../../administration/contacts/how-it-should-work.md) |
| Operator, 13 Sep (Stage 2 approval) | Scroll is the default layout in every state; the Scroll/Tabs switch stays. No Take over on a colleague's lease (a person's own second window keeps it). The Case tasks panel is not built. The Plan damage clicker ships | section row; record bar; Damage |
...
| Operator, 13 Sep | Send to EVA and Create audit are items in the Actions menu, not buttons in the record bar; the menu shows outside an edit session whenever either is available, with its editing-only items hidden | Actions menu |

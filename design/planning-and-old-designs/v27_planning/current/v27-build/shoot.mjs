// Screenshot driver for the v27 mockup: headless Chromium (the Playwright
// build) over the DevTools protocol, no npm packages. Usage:
//   node shoot.mjs <shots.json>
// shots.json: [{ "name": "01-overview", "query": "state=review", "width": 1580, "height": 1000, "full": false }]
// Every page load also reports console errors and uncaught exceptions; the
// run fails if any occur.
...
const mockup = pathToFileURL(resolve(here, '..', 'pegasus_case_record_v27.html')).href;
const outDir = resolve(here, '..', 'v27-shots');
mkdirSync(outDir, { recursive: true });
const shots = JSON.parse(readFileSync(process.argv[2] || resolve(here, 'shots.json'), 'utf8'));
const port = 9333;

const proc = spawn(chrome, ['--headless=new', '--allow-file-access-from-files', '--hide-scrollbars', '--disable-gpu', `--remote-debugging-port=${port}`, '--window-size=1580,1000', 'about:blank'], { stdio: 'ignore' });
...
for (const shot of shots) {
  ...
  const q = shot.query || '';
  await send('Page.navigate', { url: `${mockup}?strip=0&${/(^|&)(proposals|p)=/.test(q) ? '' : 'proposals=none&'}${q}` });
  await sleep(900);
  ...
  console.log(`${shot.name}.png${shotErrors.length ? '  ERRORS: ' + shotErrors.join(' | ') : ''}`);
}
ws.close(); proc.kill();
const failed = results.filter((r) => r.errors.length);
console.log(`SHOTS ${results.length} · with console errors: ${failed.length}`);

// Runs v27-selfcheck.html in the Playwright Chromium build over the DevTools
// protocol and prints its RESULT line (the --dump-dom route stalls on the
// 900 KB frame). Usage: node selfcheck.mjs
...
const page = pathToFileURL(resolve(here, '..', 'v27-selfcheck.html')).href;
const port = 9334;
const proc = spawn(chrome, ['--headless=new', '--allow-file-access-from-files', '--disable-gpu', `--remote-debugging-port=${port}`, '--window-size=1600,1100', 'about:blank'], { stdio: 'ignore' });

// v29 capture: saves real server-rendered Pegasus pages as offline state files.
//
...
for (const state of manifest.states) {
  ...
  try {
    ...
    const banner = `<!--\n  v29 baseline state "${state.id}". Captured from a running Pegasus.Web at ${manifest.source}\n`
    ...
    html = html.replace(/<head>/i, () => `<head>\n<script>window.V29_STATE=${JSON.stringify({ id: state.id, family: state.family, live: location })};</script>\n`
...
if (!only) {
  ...
  writeFileSync(join(assetsDir, 'mock', 'routes.js'),
    ...
    + 'window.V29_ROUTES = ' + JSON.stringify({ exact, shaped, transitions, families }, null, 1) + ';\n');

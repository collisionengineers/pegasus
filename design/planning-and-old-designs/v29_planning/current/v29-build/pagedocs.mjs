...
// itself (captured.json, shots.json, the v29-shots listing), so a page's list of
...
const shots = readdirSync(join(current, 'v29-shots'));
...
const shotLinks = (name, up) => [1580, 1440, 760].map((width) => {
  ...
  return file ? `[${width}](${up}current/v29-shots/${file})` : `${width} missing`;
}).join(' · ');

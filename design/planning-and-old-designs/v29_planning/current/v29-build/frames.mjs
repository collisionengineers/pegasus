...
<!--
  v29 baseline, ${family.title}. Captured from ${manifest.source}; the operator's proposals sit over it as a switchable layer.
...
<title>${escape(family.title)} · Pegasus v29 baseline</title>
<style>
...
(function () {
  ...
  var owner = {};
  var baseline = false;
  (window.V29_ROUTES.families || []).forEach(function (f) { f.states.forEach(function (s) { owner[s.id] = f; }); });
  ...
  window.addEventListener('message', function (event) {
    if (!event.data || event.data.v29 !== 'state') return;

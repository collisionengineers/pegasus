// v29 mockup shim. The only non-live script in a captured state page.
//
...
(function () {
  'use strict';
  var routes = window.V29_ROUTES || { exact: {}, shaped: {}, transitions: {} };
  ...
  function go(id) {
    ...
    if (window.parent !== window) {
      try { window.parent.postMessage({ v29: 'state', id: id }, '*'); } catch (e) { /* standalone */ }
    }
  ...
  function retarget(root) {
    ['data-select-url', 'data-href', 'data-url', 'data-row-href'].forEach(function (name) {
      root.querySelectorAll('[' + name + ']').forEach(function (element) {
        ...
        var id = resolve(live);
        element.setAttribute('data-v29-live', live);

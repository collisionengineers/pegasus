...
Status: **Selected for the planned `0.1.0-alpha.1` shell.** This approves the landing/navigation strategy over the shared complete Intake, Triage, Case and Administration flows in [the UI specification](../../product/ui-spec.md), not every raster detail and not a partial product.
...
Trade-off: strongest shared-office awareness and daily/week visibility, but depends on truthful independent queries and can become dense. It assumes no saved views, bulk actions, inline mutation, calendar, assignments or later email queues. `Next`/`unallocated`/`Later`/`unallocated` UI re-enters the route.

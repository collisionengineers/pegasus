...
- `operator-notes/` is read-only unless the user explicitly authorises its
  change. Treat `reference/` as evidence, not current product or architecture
...
- Keep status, scope, and supersession explicit. Plans are not implementation;
  a file or registration is not a call; local checks are not deployment or
  acceptance. Do not create generated status ledgers.
- Validate first-party links, anchors, commands, paths, and stated caller or

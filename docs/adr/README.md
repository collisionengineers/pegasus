...
| [ADR-0018: In-process ONNX VRM recognition engine](0018-in-process-onnx-vrm-recognition.md) | Accepted engine selection for `INT-17`: vendored, hash-pinned fast-alpr/fast-plate-ocr ONNX models via `Microsoft.ML.OnnxRuntime`; abstention over guessing, no image egress. Threshold acceptance from the operator-reviewed cohort remains open decision 1; it proves no implementation, caller, or acceptance. 2026-08-03: the operator exercised the separately gated `INT-28`/`INT-32` decision this body names — automatic Image-intake registration and unambiguous-match association run at the provisional bar pending that acceptance ([requirements](../requirements.md#ordinary-image-vrm-and-image-analysis) owns the amended boundary); the body is unchanged. |

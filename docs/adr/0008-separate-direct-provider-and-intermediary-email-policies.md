...
source. It has no provider registry, intermediary policy, live Worker caller,
...
3. Direct-provider policies and intermediary policies are separate,
   ...
   unrelated policies.
4. For intermediary mail, sender traits identify the intermediary route. The
...
5. A Collision Engineers staff forward retains its outer sender and message as
...
6. Each route policy owns its own evidence precedence and case-association

...
- A standalone Audit requires its instruction and the separate original report
  ...
  reads an unambiguous literal outcome from that report: `repairable` prefixes
  the allocated reference with `a.` and `total loss` with `ap.`. The prefixed
  value is the Case/PO itself, not a second or later reference. Missing or
  ...
  attachment; the declaration decides its prefix. Manual upload additionally
  requires explicit staff acceptance. Neither automatic route requires staff

...
A named classification remains visible when several classifications share an
aggregate workspace queue. `Other` is only the reasoned taxonomy extension for
a genuinely new classification. `Needs sorting` is an operational abstention
...
| --- | --- | --- | --- | --- |
| `General/autoreply` | Generated automatic-reply evidence; never quoted new-work text | route predicate or staff | Other | No action |
| `General/undeliverable` | Delivery-status/non-delivery evidence for the exact message | transport evidence or staff | Other | No action |
| `General/acknowledgement` | Acknowledges receipt without a request, new work, dispute, amendment, or cancellation | staff until a predicate is accepted | Other | No action |
| `General/general-chase` | General chase, including one referring to several Cases; never one-to-many association | staff | Other | Case queries |
| `General/case-summary` | Informational summary with no new instruction or actionable request | staff | Other | No action |
| `billing/payment-notification` | Payment notification, excluding a question/request | predicate or staff | Other | Billing |
| `billing/remittance` | Remittance advice/evidence, excluding a billing question | predicate or staff | Other | Billing |
| `billing/invoice-request` | Requests an invoice or invoice action | predicate or staff | Other | Billing |
| `billing/billing-query` | Asks a billing, invoice, payment, or remittance question | predicate or staff | Queries | Billing |
| `billing/general-billing` | Billing mail fitting no more specific billing subtype | reasoned staff decision | Other | Billing |
...
| `non-client-related` | Internal/company, tool, service, or software mail unrelated to client work | sender/route evidence or staff | Other | Other |
| `in-progress-cases/cancellation` | Explicit cancellation; it wins over quoted historic instructions | route predicate or staff | Other | Cancellations |
| `in-progress-cases/case-update` | Update on ongoing work, excluding new instruction/post-report challenge | staff | Other | Case updates |
| `in-progress-cases/client-chasing-for-update` | Client asks for progress on ongoing work | staff | Other | Case updates |
| `in-progress-cases/provider-chasing-for-update` | Provider asks for progress on ongoing work | staff | Other | Case updates |
| `in-progress-cases/ongoing-correspondence` | Other ongoing correspondence after more-specific subtypes are excluded | reasoned staff decision | Other | Case updates |
...
| `pre-instruction-emails/pre-formal-instruction-request` | Known pre-formal handling request, excluding Triage | staff | Other | Pre-instructions |
| `pre-instruction-emails/images-received` | Images before formal instruction, excluding an accepted instruction | attachment/route evidence or staff | Other | Images |
| `internal-cc` | Internal copied correspondence, not the primary actionable occurrence | header/recipient evidence or staff | Other | Other |
| Sent: `Report sent` | Exact sent report correspondence; classification alone does not prove delivery | immutable Sent-item evidence or staff | Other | Other |
| Sent: `case-rejected` | Exact outbound rejection | immutable Sent-item evidence or staff | Other | Other |
| Sent: `query-sent` | Exact outbound query/information request | immutable Sent-item evidence or staff | Other | Other |
| Sent: `additional-image-request` | Exact outbound request for better/additional images | immutable Sent-item evidence or staff | Other | Other |

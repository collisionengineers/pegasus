...
EVA remains authoritative for named Engineer assignment and downstream engineering until an accepted replacement. Pegasus now implements the focused manual handoff locally: `Pegasus.Core` owns Review-only generation, required-custody and current-evidence eligibility, deterministic bundle composition, frozen revisions, reasoned download, and the once-per-Case `First sent to Engineer` proxy. The authenticated Case surface and composition-gated Automation ingress call those Core use cases; EF persists revision/download truth, and no EVA network client exists. Custody retry is a separate human-only Core use case reached by the Case surface, while the Worker processes the same persisted custody work through Infrastructure adapters. Release 23 added a second, narrower act over the same archive format: an **operator export** of a case (`IExportCaseBundle`), reached from the Case action bar in Review. It is a read — no revision, no first-hand-off proxy, no case-version change, no edit lease — and it reuses `EvaBundleSchema`'s exact field order and image naming. Since ENG-014 there is one packaging and no per-caller difference: the archive is the two-space-indented thirteen-key JSON and `Images/`, and nothing else. `manifest.sha256` and `provenance.json` are no longer produced, persisted, or reconstructed — neither was an operator requirement, no importer or verifier ever read either, and all three known-good EVA samples carry no companion file. The whole-archive SHA-256 is unchanged in role and still serves as the revision `InputFingerprint` and the download `Content-Digest`; field provenance remains an in-memory guard inside `ValidateSource`, never a shipped file. It differs from the hand-off only in what counts as evidence: a suggested value travels with its real `Suggested` status, a field the case does not hold is emitted empty and marked `Unrecorded`, and an absent inspection date resolves to the export date as a named system default. The hand-off's bar is unchanged and is enforced where it always was, in `CaseEvaMapping.MapForProduction`; `EvaBundleSchema` now guards the archive format only, having previously duplicated that bar unreachably.

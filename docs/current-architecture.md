...
| Record edit scopes for Triage Cases and Image Intake records (stale same-holder re-claim, explicit Take over, beacon release) | `src/Pegasus.Core/Workflow/RecordEditScope.cs`, `src/Pegasus.Infrastructure/Persistence/EfEditScopeStore.cs`; `src/Pegasus.Web/wwwroot/js/edit-scope-release.js`. `src/Pegasus.Infrastructure/Persistence/CaseMutationAuthority.cs` gives Case custody and upload association the Case workflow or, for a Triage Case, its Triage edit scope. |

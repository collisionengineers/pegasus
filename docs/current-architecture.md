...
| Document-read phase timing | `src/Pegasus.Core/Documents/DocumentReadTelemetry.cs` defines allowlisted phases; the configured production `src/Pegasus.Web/DocumentReadTelemetryBridge.cs` emits phase/duration events through the existing sampled Application Insights pipeline. It adds no browser SDK or ingestion endpoint. Offline hosting does not compose this bridge or the production Blob document cache. |

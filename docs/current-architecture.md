...
EVA remains authoritative for named Engineer assignment and downstream engineering until an accepted replacement. Pegasus produces the EVA package locally, as one act: the operator export of a case (`IExportCaseBundle`), reached from the Case action bar. `Pegasus.Core` owns the field mapping (`CaseEvaMapping.MapForOperatorExport`), image eligibility (`EvaHandoffPolicy.SelectEligibleImages`), deterministic bundle composition (`EvaBundleSchema`) and the `First sent to Engineer` proxy port (`IEvaHandoffProxy`); `EvaHandoffStore` in Infrastructure calls them, and no EVA network client exists. The export is a `POST` with antiforgery. It does not move the case version, take an edit lease or carry an operation key; the one row it writes is the once-per-case proxy in `EvaFirstHandoffProxies`, whose primary key on the case is the once-per-case guarantee, and which two check constraints stop from claiming EVA receipt or Engineer assignment. That row feeds the dashboard's "Sent to Engineer" count. Only an unaccepted field mapping refuses an export: a case missing a field exports it empty and marks it `Unrecorded`, a suggested value travels with its real `Suggested` status, and an absent inspection date resolves to the export date as a named system default. The archive is the two-space-indented thirteen-key JSON and `Images/`, and nothing else; `manifest.sha256` and `provenance.json` are no longer produced, persisted or reconstructed, and field provenance is an in-memory guard inside `ValidateSource`, never a shipped file. The whole-archive SHA-256 still serves as the download `Content-Digest`. Until ENG-016 there was a second act beside it -- a Review-gated hand-off with frozen revisions, a reasoned download route, an Automation MCP surface and three tables of its own, and a fail-closed mapping that refused any case short of accepted evidence for all thirteen fields. All of it is gone; the export absorbed the proxy and nothing else. Custody retry is a separate human-only Core use case reached by the Case surface, while the Worker processes the same persisted custody work through Infrastructure adapters.

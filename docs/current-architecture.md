...
EVA remains authoritative for named Engineer assignment and downstream engineering until an accepted replacement. Pegasus produces the EVA package locally as one act: the operator export of a case (`IExportCaseBundle`), reached from the Case action bar. That export is the current send-to-Engineer route because staff import its package into EVA; no EVA network client exists. `Review` is the one readiness owner, and requires complete instructions and images. Export does not duplicate field, evidence-status, Case-custody, or Audit-custody gates. Suggested values travel with `Suggested` provenance; VAT and mileage are optional, mileage requires its unit when present, and a blank inspection date resolves to the export date. The antiforgery-protected POST carries a replay key, writes structured Case action history for every successful export, writes the once-per-case `EvaFirstHandoffProxies` row on the first, and returns the archive SHA-256 as `Content-Digest`; it does not take an edit lease or move the case version. Saving case data already invalidates completeness and returns the case to `Not ready`, with an operator notice. The archive is the two-space-indented thirteen-key JSON and `Images/` only. The superseded frozen-revision handoff, reasoned download route, Automation MCP surface, and three dedicated tables are removed. Custody retry remains a separate use case. Planned successors are the EVA API when usable and then direct estimating-system integrations that replace EVA; AI-generated estimates can instead remain in Pegasus for Engineer review and report generation.

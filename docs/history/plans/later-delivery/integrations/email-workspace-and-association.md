...
Features: `INT-05`; `INT-06`; `INT-07`; `MAIL-01`; `MAIL-02`; `MAIL-03`; `MAIL-04`; `MAIL-05`; `MAIL-06`; `MAIL-07`; `MAIL-08`; `MAIL-09`; `MAIL-10`; `MAIL-11`; `MAIL-13`; `MAIL-23`; `UI-10`; `UI-14`; `MCP-05` — `Next`/`unallocated`.
Pre-conversion status: **Planned.** The combined mailbox categorisation and automatic-matching dossier is an open gate for only its named categorisation/automatic-association slices.

## Boundary, dependencies and intended callers

One Core classification/association policy consumes the accepted `0.0.0-development` classification foundation and decision dossier. Worker/Graph ingestion, Web workspace and staff MCP are separate intended callers/adapters that call named Core use cases; none may host a second classifier, queue policy, case lifecycle, or sender. Dependencies include `0.0.0-development` classification evidence, `0.1.0-alpha.1` source identity/custody, identity/action history, Outlook/background processing, lifecycle, and the approved `Next`/`unallocated` UI route.

## Slices

### Ingest all four mailboxes

Features: `INT-05`; `INT-06`; `INT-07` — `Next`/`unallocated`.
...
Features: `MAIL-01`; `MAIL-02`; `MAIL-03`; `MAIL-04` — `Next`/`unallocated`.
...
Features: `MAIL-05`; `MAIL-06`; `MAIL-07` — `Next`/`unallocated`.
...
Feature: `MAIL-08` — `Next`/`unallocated`. The lifecycle Core owner supplies suggestions through the approved workspace; suggestions cannot automatically transition a case or send a message.

### Associate email and cases

Features: `MAIL-09`; `MAIL-10` — `Next`/`unallocated`.
...
Features: `MAIL-11`; `MAIL-13`; `UI-10`; `UI-14` — `Next`/`unallocated`.
...
Feature: `MAIL-23` — `Next`/`unallocated`. The accepted mailbox policy maps the taxonomy; a UI view does not redefine that policy or Triage.

### Expose classified email actions through existing staff MCP

Feature: `MCP-05` — `Next`/`unallocated`. The MCP adapter exposes only the same named Core use cases and authorisation/action history.

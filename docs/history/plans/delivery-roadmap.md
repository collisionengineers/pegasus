...
| 4 | [`0.0.0-development` shared-development target](remainder-delivery/platform/azure-observability-and-release.md#provision-and-prove-`0.0.0-development`-shared-development) | Order 3 complete, then exact subscription/tenant/environment/resource-group/SKU/cap approval | Authorised terminal previews/provisions, applies the immutable migration bundle, then deploys the same hashed Web/Worker packages through `azd` service routes | Planning does not authorise Azure read/write; deployment does not prove product acceptance or a Worker business caller |
...
| 16 | [Local inspection-address resolution](remainder-delivery/integrations/vehicle-data-and-eva-export.md#resolve-inspection-address-from-reviewed-reference-data), [vehicle/MOT lookup](remainder-delivery/integrations/vehicle-data-and-eva-export.md#look-up-vehicle-and-mot-data) and [`0.1.0-alpha.1` EVA bundle](remainder-delivery/integrations/vehicle-data-and-eva-export.md#export-the-`0.1.0-alpha.1`-eva-bundle) | Confirmed case data and separately accepted external contracts | Staff-reviewed address or exact `Image Based Assessment`; guarded lookup; successful export proxy only |
...
| Source formats | [DOC/MSG automation](remainder-delivery/integrations/source-custody-and-document-processing.md#automate-legacy-doc-and-msg) and [scan-like-PDF OCR](remainder-delivery/integrations/source-custody-and-document-processing.md#`Next`/`unallocated`-targeted-scanned-pdf-ocr) | Bounded genuine local evidence, no ordinary-image OCR conflation, one custody/processing owner |
| Matching and vision | [Image/instruction matching](remainder-delivery/casework/intake-and-case-acceptance.md#`Next`/`unallocated`-match-image-led-and-instruction-led-records) then [reviewed image assistance](remainder-delivery/casework/intake-and-case-acceptance.md#`Next`/`unallocated`-assist-vehicle-image-and-damage-review) | Accepted definitive predicate permits automatic association; otherwise explainable suggestion, staff confirmation/correction/reversal, provenance and evaluation; no second identity or silent destructive merge |

...
| [ADR-0006: Provider-neutral intake with contained QDOS policy](ADR-0006-provider-neutral-intake-with-contained-qdos-policy.md) | Accepted pre-release policy. Decision 0011 supersedes its single-policy selection and no-provider-registry/table limits. ADR-0014 supersedes only **Decision items 1 and 8** and the named opening **Limits and deferred-capability impact** exclusions for QDOS-alpha implementation. `DOC-CON-053` supersedes **Decision item 7 only where it selected Development SQLite and provider-neutral dual-provider migration behavior**: SQL Server LocalDB is the sole local provider and Azure SQL is the deployed provider. All other ADR-0006 clauses, including the unaffected parts of item 7, remain in force; provider-neutral transport, provenance, storage, and fail-closed boundaries remain unchanged. |

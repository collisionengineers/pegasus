...
Collision Engineers staff currently use an ASP.NET Core Razor Pages application. The sole caller-proved mutation is a Development-only manual intake upload. Future accepted provider API and staff MCP calls enter through separate Web boundaries. The .NET 10 isolated Azure Functions Worker is the intended mailbox and background composition root, but currently has no trigger, input, or Core caller.
...
### Caller-proved product paths

- `POST /Intake/Upload` is the sole proven mutating product entry point. It is available only under the Development-only local-intake gate.
...
- The browser is the real caller; processing and persistence logic are not duplicated in the PageModel.
...
A caller may upload one file, up to 10 MB, with one of these extensions:

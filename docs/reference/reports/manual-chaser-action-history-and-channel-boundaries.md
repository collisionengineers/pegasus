...
- The WhatsApp operator note says WhatsApp is primarily used to chase garages for images.
- The questionnaire already requires recurring seven-day reminders while material is missing, manual copy-and-paste messages for email or WhatsApp, and an optional Box File Request. Automated sending is outside the `0.1.0-alpha.1`.
...
The lifecycle and work-management plan already plans one Core due/chaser policy, reminder schedule/history, a copyable Web action, and a later Worker caller. It explicitly says copying sends nothing and does not prove delivered communication.

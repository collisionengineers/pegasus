...
- The operator process includes report preparation followed by post-report queries and disputes.
- The questionnaire requires the first release to progress through inspection/report preparation and post-report work. A case closes through post-report completion, provider cancellation, Collision Engineers rejection, or the distinct wrong-principal outcome `Created in error`.
- The remainder-delivery finish line already carries operators through report and post-report activity. This is broader and more precise than stopping at EVA handoff.
...
The lifecycle and work-management plan names planned Core `CaseLifecycle` and `CaseWork` policies, guarded Web actions, and future Worker callers. It records `Report sent` only from an explicitly associated exact Sent item and distinguishes it from later post-report completion. Principal/reference immutability is already required at reference allocation.

The exact sent-evidence source and time are settled. The combined mailbox categorisation and email-matching research must still determine:
...
The Outlook plan keeps Sent Items access outside its current inbound-Inbox implementation slice until the separate permission boundary is approved. The Box plan owns custody, not report-completion policy. The EVA plan provides a future manual download bundle and deliberately emits no direct EVA adapter task until its mapping and readiness procedure are accepted.

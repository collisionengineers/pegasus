...
**Operator decision:** Accepted in narrowed form on 2026-07-24. `fast-alpr` is recorded as a potential future vehicle-registration recognition candidate, not as the selected v2 engine.
...
This report accepts a future product and evidence boundary. It does not move automated VRM reading or AI/vision into the first MVP, select a provider or model, authorise image upload to any external service, or prove an implementation.
...
This narrows the predecessor's useful suggestion-first principle to current v2 ownership. Business decisions remain in a named Core use case; an Infrastructure image-analysis adapter may return typed observations but cannot decide workflow.
...
- The [source-custody plan](../../../plans/remainder-delivery/integrations/source-custody-and-document-processing.md) keeps AI/vision and VRM OCR deferred, retains engine-neutral source identities, and deliberately adds no dormant model, client, flag, queue, or widened OCR route.
- The [vehicle and EVA plan](../../../plans/remainder-delivery/integrations/vehicle-data-and-eva-export.md) permits future typed, staff-visible suggestions but forbids silent overwriting of case data.

The current evidence state is therefore **Planned/deferred**, not implemented or called. The Development-only `/Intake/Qdos` path retains ordinary image evidence; no current v2 VRM-recognition or image-analysis caller is established by these legacy files.
...
- using the predecessor's small decision-layer harness as proof of raw model accuracy or v2 suitability.

The legacy statement that original images remain immutable also differs from current v2 policy. Current v2 retains every previous version, uses logical removal, audits revisions, and makes files application-read-only when a case closes until an authorised reopen. Immutable source identity and retained history are required; irreversible evidence immutability is not.
...
Activation requires a direct product-scope decision, accepted data contract, representative evaluation, data-protection and residency approval, cost/licence review, a real intended caller, and—if the architecture boundary changes—an accepted current v2 ADR.

...
**Updated 2026-07-22** for two image rows only: `IMAGE_ROLE_CLASSIFY_ENABLED` and `PLATE_OCR_ENABLED`
were re-read live and then deliberately turned **off** at 2026-07-22T10:56:13Z by explicit operator
direction ("disable this and any other image classification system"). `IMAGE_ANALYSIS_ENABLED` and
`MCP_IMAGE_INGEST_ENABLED` were re-read and were already **off**; `OCR_SCANNED_PDF_ENABLED` and
`TRIAGE_IMAGES_ROUTING_ENABLED` were deliberately left **on** (they are document-text extraction and
email routing respectively, not image classification). Every other row still carries its earlier reading.
...
| `INTAKE_ENGINE_ENABLED` | The from-scratch rebuilt intake rules engine (`@cs/intake-engine`). Two things change: the system works out WHO sent an instruction by reading the original sender out of a forwarded email's quoted header (rather than the staff member who forwarded it, who matches nothing), and it decides what KIND of job the email is (standard / audit-repairable / audit-total-loss / report+audit) from the provider's own phrase list. | A staff-forwarded instruction resolves to its real provider, and the case type comes from the new engine. | Provider matching sees only the forwarding staff address (so a forward matches nothing), and case type comes from the parser/classifier signals as before. | **ON** (`cespk-orch-dev`) — deployed and flipped from its ship-dark default at 2026-07-22T12:35:47Z. Kill-switch: set false to fall back to the previous behaviour, no deploy needed. Falls back to the old decision by itself whenever the engine cannot resolve (unknown sender, ambiguous domain, or an audit whose verdict it cannot determine), so the worst case is the previous behaviour. Known limit: the engine has no `diminution` concept, so an existing diminution decision is preserved rather than overridden. |
...
| `PLATE_OCR_ENABLED` | Whether the system tries to automatically read a number plate out of a photo. | Plates in photos are auto-read into text. | No automatic plate reading. | **OFF** (`cespk-orch-dev`) — flipped from ON at 2026-07-22T10:56:13Z by the same operator direction as `IMAGE_ROLE_CLASSIFY_ENABLED`. Reversible in one command, no deploy. Note this was also the *fallback* path: `extractImages` used plate OCR whenever the classifier was off or abstained, so with both off no registration is read from a photo at all. |

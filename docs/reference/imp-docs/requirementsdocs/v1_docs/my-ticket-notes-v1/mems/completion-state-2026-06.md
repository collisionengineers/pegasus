...
- **Two baselines.** Baseline A = `0.1.0-alpha.1` feature-parity behind `Next`/`unallocated` contracts (EPIC-00..11 + docs/plans/implementation-todos.md): **~82% complete**. Baseline B = the forward revamp vision (investigation/07 Phases 0-4 + audit case-type): **~8% complete** (only audit *detection* shipped). A blended "75%" is misleading.

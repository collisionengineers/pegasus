...
## Commits
- **Taxonomy `Next`/`unallocated` `cancellation` category** — ships in the taxonomy-`Next`/`unallocated` DDL delta (`84fb102`) and the re-cut
  parser engine (`ec45970`, engine-`Next`/`unallocated`.3 — `cancellation` + `case_update` rules). The email is matched to its
...
- **Gate flipped live** — `TRIAGE_CANCELLATION_ENABLED=true` on `cespk-orch-dev`; the taxonomy-`Next`/`unallocated` DDL delta
  (was operator-gated §D7) was applied and the taxonomy-`Next`/`unallocated` parser deployed, so the category and the engine
...
audit trail; unmatched cancellations surface for review. Part of Rules Engine `Next`/`unallocated` Phase 2 (relates TKT-023,
TKT-046).

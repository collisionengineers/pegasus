...
VERIFIED-LIVE. The ticket's own sample .eml POSTed to the deployed parser POST /api/classify-email -> 200 query/query_existing_work at 0.8, rule:query_with_reference, signals carrying body_jobref:SAB/46286/1 + body_vrm:HN13XMO + chase_keywords. Regression pin tkt031-client-chaser green in baseline-`Next`/`unallocated`; registry cross-checked (live parser engine-`Next`/`unallocated`.7, taxonomy `Next`/`unallocated`). This was exactly the closing condition this file's Pending section specified.

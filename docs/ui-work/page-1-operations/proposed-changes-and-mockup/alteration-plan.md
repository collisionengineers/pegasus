...
- The "Review" tile is backed by the wrong entity: it renders `@Model.Counts.DraftReady`, an
  intake-receipt count (`asp-route-decision="draft_ready"` links to the received-items list), not
  the count of cases in the `Review` stage. A case-queue tile backed by an intake query is a
  correctness bug, not a styling issue. The count is also cumulative for all time — acceptance
  never decrements it (`EfIntakeReceiptStore.GetCountsAsync:152-164` applies no acceptance filter;
  `EfCaseAcceptanceStore.cs:276-288` leaves `receipt.Decision` untouched).
- Deeper: `DraftReady` should not exist. `requirements.md:251` — "the allocation decision adds no
  universal manual acceptance gate" — means definitive authorised intake creates the case directly,
  and `operator-notes.md:204` sends only ambiguous or unidentified material to `Needs sorting`. Most
  of what this tile counts should never have been a queue. See `defects-and-non-functional.md` §B4.
...
5. **Fix the Review tile's backing query.** It must count cases in the `Review` stage, not
   `DraftReady` intake receipts (see Dependencies). The intake-side count is not relabelled or
   relocated — it retires with the acceptance gate (`defects-and-non-functional.md` §B4).
...
8. **Remove the "Email: Received and Sent" card** and its copy *"Mailbox outcomes and owned
   ...
   explicitly unwanted; the E-mail activity section covers the need. Its tiles link into Inbox,
   which now absorbs the former Email operations screen (page 2 plan) — there is no separate
   drill-down page to route to.
...
- **New Core count queries**: cases in Not ready, cases in Review, cases in Held (the current
  ...
  reuses the `DraftReady` intake count and must be rewired to the new case-stage count.
- **Removal of the manual acceptance gate** (`defects-and-non-functional.md` §B4). While every case
  still requires a human to press "Accept and allocate case reference", the Active cases tiles
  under-count by whatever is stranded in the intake queue, so Not ready / Review / Held cannot read
  true until definitive intake creates cases directly.
...
- ~~Destination of the Email operations drill-down~~ — settled 2026-08-04: it merges into Inbox,
  so the E-mail activity tiles link to Inbox tabs and filters. Remaining sub-question: does the
  "Needs sorting" tile link to the Inbox filter and the (new) "Failed" count to the Failed
  filter, or does one combined tile serve both?

...
All three use the same complete Intake, Triage, Case and Administration flows. The user explicitly authorised generation of all three reviewed comparison rasters on 2026-07-26; they are retained under [design references](../../design/references/README.md). The remaining choice blocks final V1 UI handoff only; it does not block domain, data or integration work. Explicit user selection is still required, and selecting a shell does not approve every raster detail. Any V2/V3/V3+ UI change re-enters the complete design route rather than inheriting the V1 choice.

...
7. Web telemetry exporter (OPS-07) and minimum cutover alerts (Box custody failure, intake poison, chaser sweep), then the cutover date: all new QDOS instructions enter Pegasus; watch alerts and telemetry daily for the first week. **At this date the additive-migration requirement starts binding** — the pre-cutover exemption in [the runbook's rollback step 3](runbook.md#previous-artifact-rollback-web-and-worker) ends, because from here a rollback has live case work to preserve.

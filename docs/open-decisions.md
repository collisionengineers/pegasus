...
1. **`INT-31` upload-link limits** — **Partially settled 2026-08-29.** Still
   ...
   **A version change is not yet a migration.** Two mechanisms invalidate every
   ...
   The decided target, confirmed 2026-09-01 (EPIC-011 D20), is **100 MB per
   file and approximately 200 MB per multipart request**; the Provider API
   envelope stays 30 MB. Raising the per-file cap to 100 MB is [[INTK-052]],
...
| Report wording outside the approved assessment baseline | The `rendererref1` assessment wording is accepted for draft generation. Salvage Categories A/B/N/A wording, recovery/storage wording, and a final statement of truth remain absent or unaccepted. Engineer identity is no longer a blocker: any Engineer-role user may issue a report under typed identity only, and the supplied signature assets are governed but inactive (settled 2026-09-01, EPIC-011 D18; owner [FRD-11 § Initial renderer activation](frd/frd-11-reports-correspondence-and-reviewed-proposals.md#initial-renderer-activation)). | Unsupported reports could contain incomplete, unauthorized, or inconsistent statements. | Keep absent wording unavailable; fail closed and never infer it from signatures or samples. | Has the exact missing wording needed by the family being activated been supplied and accepted? |
...
| The Suggestions screen's fate and the remaining PAV-slider presentation questions at the UI-15 re-entry review: repurpose the built Suggestions markup as a read-only automation-change review or retire it; confirm placement and step/rounding; resolve the recorded `.send-action` contrast shortfall (2.3–4.2:1 vs 4.5:1) before any activation puts the control in front of staff; ratio basis and threshold source (per-principal or per-instruction; QDOS 80% is the only evidenced example). The requested value itself is settled — optional, 0–100 %, no default, derived visibly from the recorded Engineer's Value, proposal guidance only (settled 2026-09-01, EPIC-011 D24; owner [FRD-11 § AI Job List](frd/frd-11-reports-correspondence-and-reviewed-proposals.md#ai-job-list)). | Unresolved presentation decisions block staff-facing activation, not gated local work. | Decide at the UI-15 re-entry review; keep the control a review aid that writes nothing. | What does the UI-15 re-entry review accept for the Suggestions screen, the placement, the contrast fix, and the threshold source? |

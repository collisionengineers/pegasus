...
| INT-17 | Automatic vehicle-registration reading from ordinary vehicle images | Now | 0.1.0-alpha.1 | [Ordinary-image VRM and image analysis](requirements.md#ordinary-image-vrm-and-image-analysis) | Allocated but non-blocking for `0.1.0-alpha.1` acceptance; the engine is the in-process ONNX stack of [ADR-0018](adr/0018-in-process-onnx-vrm-recognition.md), scanning image-only material automatically in the intake pipeline, with threshold acceptance still open (open decision 1). Source-image-bound with recorded abstention/failure outcomes; no instruction invention or external upload, and the only autonomous actions are the operator-directed `INT-28`/`INT-32` register+associate at the provisional bar. |
...
| INT-28 | Automatic matching of image-led and instruction-led records | Now | 0.1.0-alpha.1 | [Matching conflicts and reversible association](requirements.md#matching-conflicts-and-reversible-association) | Operator-directed 2026-08-03: automatic registration and association run at the provisional recognition bar (open decision 1 still owns threshold acceptance). Automatic association requires exactly one eligible pre-report case with the confirmed registration and no contradictory identity evidence; anything else stays a reasoned staff decision. |

...
- **Confirmed facts:** required tiles are `Not ready`, `Review`, `Held`, `Receiving work`, `Queries`, `Other`, `Needs sorting`, `Blocked intake`, `In today`, `Submitted today`, and `Cleared this week`; every tile opens its view.
...
- **Included:** required dashboard tiles, exact filters, manual refresh, last-updated and intake retry entry point.
...
- **Decision required before implementation:** lifecycle reopen, chase cadence and report-freeze actions remain linked to [open decisions](../../open-decisions.md); this UI must not make them available prematurely.
...
- **Operator surface and observability:** exact labelled filters, readable origin and audit history; content-safe search/action telemetry. Alias search remains absent while principal correction is withheld.
...
- **Excluded:** financial editing, EVA API, report send, reopen/chase/report-freeze actions while gated, public/external views and permanent deletion.

### Implementation checklist

- [ ] Add one authorised Core query/read model covering the stated filters and durable source-origin history; add alias search only with the later settled correction slice.
- [ ] Build list/detail pages that delegate every mutation to the named owner, require the exclusive lease/version contract and hide decision-gated actions.
- [ ] Add necessary database indexes with migration evidence; do not introduce an independent search store.

### Validation checklist

- [ ] Test every emitted filter singly and in combination, date boundary and case origin after merge/reversal; do not fabricate alias fixtures before correction is settled.
...
- **Irreversible risk:** none; case/source/audit history remains retained.

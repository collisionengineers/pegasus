...
- Start v2 without importing predecessor cases, users, audit records or application state. Predecessor retirement remains a separate exact-target decision.
...
- Case acceptance writes the state change, reference allocation, case, audit event and outbox atomically. Sequence numbers never wrap, widen or return to the pool.
...
|---|---|---|---|---|
| 1 | [Relational intake draft](casework/intake-and-case-acceptance.md#review-and-resolve-an-intake-draft) | Current local slice | Existing Development-only `POST /Intake/Qdos` and `GET /Intake/Review` | Source-identity receipts and typed read-only drafts; no case/reference |
| 2 | [Staff identity, authorisation and audit](identity-and-access/staff-identity-authorisation-and-audit.md#authenticate-staff-and-enforce-role-boundaries) | Relational draft | Planned authenticated Web pages | Trusted actors and protected staff operations |
| 3 | [Principal and operational configuration](identity-and-access/staff-identity-authorisation-and-audit.md#administer-principals-and-live-operational-configuration) | Staff identity/audit | Planned Administrator pages | Stable QDOS principal and completeness policy |
| 4 | [Durable source receipt and staging](integrations/source-custody-and-document-processing.md#durable-source-receipt-processing-and-custody-hand-off) | Relational draft and actor contracts | Existing Web caller first; Worker/API planned | Production-safe original staging and custody outbox |
| 5 | [Case identity and references](casework/case-identity-and-references.md) | Configured principal, trusted actor and reviewed draft | Planned acceptance command | One active allocator; correction/aliases remain withheld by the sent-report decision |
| 6 | [Definitive case acceptance](casework/intake-and-case-acceptance.md#accept-a-definitive-case-transaction) | Orders 1-5 | Planned authenticated review action | One atomic case/reference/audit/outbox transaction |
...
| 8 | [Exclusive case editing](casework/case-editing-concurrency.md) | Staff identity, accepted case, audit and first named mutation | Planned authenticated edit mode | One active editor with stale-write protection |
| 9 | [Lifecycle and work management](casework/lifecycle-and-work-management.md) | Case identity and edit guard; open decisions as applicable | Planned guarded Web actions and Worker reminders | Permitted reviews and terminal workflow; withheld transitions stay absent |
| 10 | [Operator workspace](casework/operator-workspace.md) | Case queries, permitted lifecycle, edit guard and document links | Planned authenticated Razor Pages | Searchable operational workspace; undefined activity tiles withheld |
...
| 12 | [Vehicle data and EVA export](integrations/vehicle-data-and-eva-export.md) | Confirmed case data and accepted vendor/export contracts | Planned guarded Web commands | Vehicle enrichment and manual EVA hand-off |
| 13 | [Provider submissions](integrations/provider-submissions.md) | Accepted versioned wire contract, intake policy and audit actors | Planned provider endpoints | Principal-scoped machine intake |
| 14 | [Staff MCP](integrations/staff-mcp.md) | Existing staff Core use cases, auth and edit guard | Planned `/mcp` endpoint | Attributed staff automation without a second rule engine |
| 15 | [Azure, observability and release](platform/azure-observability-and-release.md) | Caller-backed application slices | Planned shared-development and production deployments | Managed runtime, recovery and release evidence |
| 16 | [Acceptance and cutover](platform/acceptance-and-cutover.md) | All required first-release areas | Actual Web/Worker/API/MCP journeys as delivered | Operator acceptance and controlled production cutover |

Provider API and staff MCP may branch after shared Core and authorisation contracts stabilise. Their edits to shared Web composition are integrated by one composition owner. Open decisions block only the behavior named by the [register](../open-decisions.md); a fully blocked outcome has no implementation task until its authority exists.
...
| Outlook or Exchange access | Per-environment service principal and exact mailbox/Inbox pair | Exchange Application RBAC is the sole `Application Mail.Read` grant, no unscoped Entra Graph application mail permission remains, and the adapter enforces that environment's pair. Production is `instructions@collisionengineers.co.uk` Inbox; development needs a separately approved non-production pair or stays live-disabled |

...
Status: **Hardened execution plan — source execution is gated on the exact accepted documentation-centralization and capability-versioning heads; every external target/read/write remains separately exact-approval gated.**

This plan replaces the stale execution baseline that documentation centralization distills into `docs/changes/2026-07-27-qdos-alpha-reference-corpora.md`; that existing change record remains the sole plan/evidence/outcome owner and no second status ledger survives. The exact forbidden prefix is `docs/reference/imp-docs/`: prune it before path enumeration, stat, read, relationship, search, hash, manifest, release-input, link or anchor work. Nothing below it is instruction, evidence, provenance, implementation input or a requirement source.

## 1. Goal, finish lines, and evidence language

Deliver all source, migrations, local adapters, accepted-contract live adapters, staff UI, Worker callers, MCP surface, infrastructure, operations documentation and verification needed by the 128 unique `Now` capabilities allocated to `0.1.0-alpha.1`. The implementation runs from a new Windows worktree, preserves one independently working commit per checkpoint, and ends with one open PR in `collisionengineers/pegasus`. It targets `main` only when both prerequisite changes are merged; otherwise it is stacked on the exact capability-versioning prerequisite head and records the merge order. The agent never merges a pull request.
...
2. **Released-alpha finish line (post-PR, maintainer/release-owner work):** the reviewed change is merged by a maintainer; exact-artifact validation remains applicable; the `docs/operations.md` procedure governed by ADR-0009 executes under separate write approval; the isolated Azure targets pass live dependency/recovery/capacity smoke; staff complete the real QDOS journey; Collision Engineers management accepts it; a narrow release-evidence PR lands; the final gate passes; and `0.1.0-alpha.1` tags that evidence commit. A source PR cannot truthfully prove deployment or acceptance.
...
|---|---|---|
| Capability allocation | The accepted prerequisite `docs/capabilities.md` declares 229 unique IDs: 128 Now at `0.1.0-alpha.1`, 32 Next and 40 Later at exact future targets, and 29 Not planned/unallocated. | Alpha implementation must consume the exact conflict-free prerequisite head and prove that no future-target or boundary ID gains an alpha caller/UI/schema. |
...
The new worktree must re-run the provider package and current tests before relying on historical results. It must not replace a valid immutable package merely to create a checkpoint. Commit `467284f`, old branch names and fixed worktree paths are historical proposal evidence only. Activation records the exact accepted documentation-centralization head and the exact accepted capability-versioning child head; the implementation branch starts from that child head whether stacked or already merged to `main`.
...
31. **Functions/application storage isolation:** supersede the one-account target. Functions host identity requires account-scoped Blob ownership, so sharing that account with the Web authentication ring would make the required Worker-denial proof impossible. Use one Standard LRS transport/deployment account and one Standard LRS application custody/protection account per environment; both remain public-service-endpoint, shared-key-disabled and Entra/RBAC-only. This adds no application store abstraction or deployment unit and is included in the measured cost/restore/alert gates.


### 3.1 ADR-0014 clause-level supersession record

Create `docs/decisions/ADR-0014-qdos-alpha-implementation-contract.md` in the documentation-centralized layout and update the decision index. It is an accepted addendum only after review; it must not edit accepted ADR bodies, mark an entire earlier record superseded, or use broad wording such as "the alpha plan controls." Record every overlap as follows:

| Earlier accepted clause | Alpha clause that supersedes it | Preserved remainder |
|---|---|---|
| ADR-0002's earlier absence of explicit alpha Identity lockout/reset/session values | Decisions 2-5 and checkpoint 3 set the exact lockout, recovery, reauthentication, lease and session contracts. | Identity ownership, application-managed accounts, hashing, cookies and roles remain accepted. |
| ADR-0002/ADR-0005 passages that leave vehicle-registration recognition to later work or unexecuted evaluation | Decisions 1 and 15 plus checkpoint 9 activate only the accepted QDOS VRM slice after genuine-corpus holdout acceptance. | ADR-0005 format, provenance, OCR-candidate and manual-review rules remain accepted; no vehicle category/economics inference is activated. |
| ADR-0002's combined provider-API/MCP client model | ADR-0004 already supersedes that exact combined model; Decision 9 and checkpoint 6 narrow only ADR-0004's "intake-oriented actions" wording to the static alpha MCP manifest and current-user authorization matrix. | Provider API remains a future release at its canonical target; ADR-0004 OAuth/resource/audience/current-role rules remain accepted. |
| ADR-0002 target/runtime wording that does not distinguish infrastructure, migrator and application SQL principals | Decisions 17-18 and checkpoints 12/14 define user-assigned Web/Worker runtime identities plus separately approved Entra SQL administrator group and migration runner. | One database per environment, EF Core/SQL, managed identity and the modular-monolith boundary remain accepted. |
| ADR-0002's 30-day Azure telemetry draft/default | Decision 20 and checkpoint 12 set 31-day interactive retention, sampling and a measured daily cap. | Application Insights/Log Analytics ownership and telemetry categories remain accepted. |
| ADR-0009's generic immutable migration-bundle wording where the execution RID is not fixed | Decision 19 and checkpoints 12/14 require a self-contained `win-x64` EF bundle for the authorized Windows terminal; Web/Worker packages stay `linux-x64`. | Direct-terminal order, exact approval, build-once/deploy-same-artifact, no `azd up`, and rollback-without-down-migration remain accepted. |
| ADR-0009's unresolved SQL Entra directory-resolution route | Decisions 17-18 and checkpoints 12/14 replace it only after the exact administrator group and migrator identity are approved and bootstrap proof passes. | Missing exact target/identity approval remains a stop condition, not a fallback license. |
| ADR-0002/ADR-0009 target drafts that leave remote-build behavior unresolved | Checkpoints 12/14 remove `SCM_DO_BUILD_DURING_DEPLOYMENT=true`, package locally, hash once and deploy only those packages. | Existing F1/B1/Flex target topology remains intended until exact-target validation proves it. |
| ADR-0002/ADR-0009 deployment draft's post-provision broad SQL grant path | Decisions 17-18 and checkpoints 12/14 remove the `azure.yaml` grant hook and `Grant-DatabaseAccess.ps1`; the separate exact-target bootstrap command may create only approved runtime users/roles. | IaC provisioning and direct-terminal release remain accepted; application startup never migrates or grants. |
| ADR-0002/ADR-0005 unimplemented Document Intelligence target material | Decision 16 and checkpoint 12 remove the dormant resource, parameter and role from alpha IaC because the accepted PdfPig mechanism meets the gate. | ADR-0005's visible OCR-candidate and future-alternative evidence seams remain, with no dormant cloud adapter. |
| ADR-0002's target cost estimate without enforceable alert resources | Decisions 20-21 and checkpoint 12 add the scoped budget/alerts and exact measured inputs; alerts never stop resources automatically. | The accepted combined Development/Production envelope remains the planning constraint. |
| ADR-0004's static MCP transport without concrete abuse ceilings | Decision 28 and checkpoints 3/6 set exact endpoint/tool throttles and fail-before-mutation behavior. | OAuth flow, registered callback, metadata, resource indicator, token validation and current-user policy remain accepted. |
| ADR-0006's Development-only `/Intake/Upload` caller limitation | Checkpoints 5/7 activate accepted Graph/Worker and production staff intake callers only after custody, principal, provider-policy and failure-evidence gates pass. | `ProcessIntake` remains the sole Core intake owner; provider-neutral transport/storage, source-occurrence identity and no-normal-startup-migration rules remain accepted. |
| Earlier same-process bootstrap secret transport proposal | Decision 29 and checkpoints 2/3/14 require concealed same-process stdin and prove no argument/environment/file/output/log/history copy. | One-shot, singleton, fail-closed initialization remains accepted. |
| Repository boundary before a root .NET tool manifest existed | Decision 30 and checkpoints 2/14 permit only `.config/dotnet-tools.json` for pinned EF tools. | No new project, runtime, deployment unit or general configuration root is created. |
| ADR-0002/ADR-0009 one-storage-account target wording | Decision 31 and checkpoint 14 split Functions transport/deployment from application custody/protection because the host-required account-scoped Blob owner grant cannot coexist with Worker denial to the Web authentication ring. | Standard LRS, public-service-endpoint, managed-identity/RBAC-only storage, exact target approval, cost and restore gates remain accepted. |

ADR-0014 must also list every reviewed overlap found during implementation and state "no overlap" for ADR-0001, ADR-0003, ADR-0007, ADR-0008, ADR-0010, ADR-0011, ADR-0012 and ADR-0013 after exact-head review. Any additional overlap is a stop condition until its exact superseded clause and preserved remainder are recorded.
...
- Identity/administration: `InitializeApplication`, `CreateStaffAccount`, `DisableStaffAccount`, `AssignStaffRoles`, `ReviewStaffAccess`, `CreateOrganization`, `UpdateOrganizationRoles`, `CreatePrincipal`, `ReplacePrincipal`, `UpdateWorkflowConfiguration`, `UpdateApprovedMailbox`, `RegisterPublicMcpClient`, `RevokePublicMcpClient`, `RevokeStaffMcpAuthorizations`. An organization has independently selectable `WorkProvider` and `InstructionIntermediary` roles; a principal belongs to exactly one organization and replacement preserves the disabled predecessor identity.
- Cases: `ConfirmCompleteness`, `AcquireCaseEditLease`, `RenewCaseEditLease`, `ReleaseCaseEditLease`, `SaveCase`, `HoldCase`, `ReleaseCase`, `TransitionCase`, `CloseCase`, `ReopenCase`, `ArchiveCase`, `CreateLinkedReplacement`, `CreateCaseTask`, `AssignCaseTask`, `CompleteCaseTask`, `CancelCaseTask`, `RecordEngineerFinding`.
...
- Documents/integrations: `DownloadIntakeSource`, `AddCaseDocument`, `DownloadCaseDocument`, `ExportCaseDocuments`, `CreateBoxFileRequest`, `RevokeBoxFileRequest`, `CreateRequestUploadLink`, `RevokeRequestUploadLink`, `UploadToRequest`, `LogicallyRemoveDocument`, `RequestVehicleLookup`, `AcceptVehicleSuggestion`, `GenerateEvaHandoff`, `LinkReportEvidence`, `UnlinkReportEvidence`.
- Background: `RetryMailboxProcessing`, `RetryExternalWork`, `PollApprovedInbox`, `PollSentEvidence`, `DispatchPendingWork`, `ProcessQueuedIntake`, `ProcessQueuedExternalWork`, `ProcessQueuedCustody`, `ProcessQueuedVehicleLookup`, `ReconcilePoisonedQueueWork`, `RunDueChasers`, `ReconcileStagedArtifacts`. The `external-work` dispatcher loads the ID-only durable work row, rejects unknown kinds, and invokes exactly one custody or vehicle handler. Poison reconciliation loads an ID-only poison message, atomically marks its durable row terminal/idempotent before deleting it, and retains malformed-ID evidence only as a content-safe hash/security event. Each Functions trigger calls exactly one Core contract and carries IDs only.
- Queries: `GetOperationsSnapshot`, `GetEmailOperations`, `GetRequestOperations`, `ListIntake`, `GetIntake`, `SearchCases`, `GetCase`, `ListTriage`, `GetTriage`, `ListOrganizations`, `GetOrganization`, `ListStaffAccounts`, `GetStaffAccount`, `GetAccessReview`, `GetRoleAssignments`, `GetWorkflowConfiguration`, `ListApprovedMailboxes`, `GetRequestUpload`; contracts return authorised immutable projections and Infrastructure implements their indexed reads.
...
- The evidence campaign freezes genuine human-reviewed cohorts and untouched holdouts for only the activated QDOS direct/intermediary routes, extraction, Triage and exact-report matchers; it proves positive, negative, ambiguous, forward, collision, retry, correction/reversal and holdout behavior. Additional-provider capabilities retain their exact future targets and are not activated or revived through the superseded 88/56-candidate campaigns. Commit only aggregate counts, input hashes, policy versions and limits.
...
- Add exactly the nine Functions in the caller matrix below. Inbox and Sent polls have a configurable one-minute default. SQL leases/cursors prevent overlap per mailbox or reconciler; a delayed tick/restart resumes from the durable cursor, and acknowledgement follows only a durable idempotent result. No trigger binds a service/repository directly or calls a second Core use case.
...
#### 5.2.1 Exact Worker caller matrix

| Physical function class | Binding/queue | Sole Core contract | Durable input and observable outcome |
|---|---|---|---|
| `InboxPollFunction` | Timer, default every minute | `PollApprovedInbox` | Approved-mailbox ID/cursor; advances only after immutable receipt/outcome, otherwise visible retry/failure. |
| `SentEvidencePollFunction` | Timer, default every minute | `PollSentEvidence` | Approved-mailbox ID/cursor; records immutable Sent evidence or a bounded visible mismatch/failure. |
| `PendingWorkDispatchFunction` | Timer | `DispatchPendingWork` | Claims due outbox IDs and writes only ID-only queue messages idempotently. |
| `IntakeWorkFunction` | `intake-work` queue | `ProcessQueuedIntake` | One receipt GUID; durable intake state owns attempts/outcome before acknowledgement. |
| `ExternalWorkFunction` | `external-work` queue | `ProcessQueuedExternalWork` | One outbox-work GUID; Core rejects unknown kind and selects exactly one custody or vehicle handler. |
| `IntakePoisonFunction` | `intake-work-poison` queue | `ReconcilePoisonedQueueWork` | One receipt GUID; atomically records terminal operator-visible state before message deletion. |
| `ExternalPoisonFunction` | `external-work-poison` queue | `ReconcilePoisonedQueueWork` | One outbox-work GUID; same idempotent mark-before-delete contract, scoped by queue kind. |
| `StagedArtifactReconciliationFunction` | Timer | `ReconcileStagedArtifacts` | Bounded durable-ledger batch; completes/tag-cleans or retains visible failure without guessing ownership. |
| `DueWorkSweepFunction` | Timer | `RunDueChasers` | Injected-clock due range; records each due/chaser transition and schedules external work idempotently. |

`Program.cs` registers only these function callers and their composition dependencies. `host.json` fixes queue visibility, `maxDequeueCount=5`, bounded concurrency and poison behavior; local and target configuration supply schedules/queue names without changing the matrix. Real-host tests enumerate function metadata and fail on a missing/extra binding, a direct Infrastructure business call, source bytes in a message, or a function-to-contract mismatch.
...
- The Box CLI is authenticated, and creation of case files is in scope only under folder ID 392761581105; no other Box folder/root is authorized.
...
### 5.9 Exact alpha MCP manifest

The implementation stores one static, ordered manifest beside the Web transport; discovery, endpoint registration, authorization and contract tests consume that same object. No reflection, assembly scan or separately maintained list may add a tool. Each mapping below is one tool → one Core contract; schemas require actor-derived authorization, current version/lease where applicable and idempotency keys on mutations.

| Tool → sole Core contract | Scope/policy | MCP hints `(readOnly, destructive, idempotent, openWorld)` |
|---|---|---|
| `operations.get` → `GetOperationsSnapshot`; `intake.list` → `ListIntake`; `intake.get` → `GetIntake`; `cases.search` → `SearchCases`; `cases.get` → `GetCase`; `triage.list` → `ListTriage`; `triage.get` → `GetTriage` | `pegasus.read`; current-user record authorization | `(true,false,true,false)` |
| `intake.resolve` → `ResolveIntake`; `intake.reevaluate` → `ReevaluateIntake`; `cases.save` → `SaveCase`; `cases.acquire_edit_lease` → `AcquireCaseEditLease`; `cases.renew_edit_lease` → `RenewCaseEditLease`; `cases.release_edit_lease` → `ReleaseCaseEditLease`; `cases.create_task` → `CreateCaseTask`; `cases.assign_task` → `AssignCaseTask`; `triage.assign` → `AssignTriage`; `triage.unassign` → `UnassignTriage`; `triage.record_finding` → `RecordTriageFinding`; `triage.supersede_finding` → `SupersedeTriageFinding`; `triage.link_response` → `LinkTriageResponseEvidence`; `triage.unlink_response` → `UnlinkTriageResponseEvidence`; `triage.link_case` → `LinkTriageCase`; `triage.unlink_case` → `UnlinkTriageCase` | `pegasus.write`; named command policy and current role | `(false,false,true,false)` |
| `intake.accept` → `AcceptIntake`; `intake.link_case` → `LinkIntake`; `intake.unlink_case` → `ReverseIntakeLink`; `cases.confirm_completeness` → `ConfirmCompleteness`; `cases.hold` → `HoldCase`; `cases.release_hold` → `ReleaseCase`; `cases.transition` → `TransitionCase`; `cases.close` → `CloseCase`; `cases.reopen` → `ReopenCase`; `cases.archive` → `ArchiveCase`; `cases.create_linked_replacement` → `CreateLinkedReplacement`; `cases.complete_task` → `CompleteCaseTask`; `cases.cancel_task` → `CancelCaseTask`; `cases.record_engineer_finding` → `RecordEngineerFinding`; `triage.complete` → `CompleteTriage`; `triage.cancel` → `CancelTriage`; `triage.reopen` → `ReopenTriage`; `documents.logical_remove` → `LogicallyRemoveDocument` | `pegasus.write`; consequential-action policy, reason/confirmation where the Core contract requires it | `(false,true,true,false)` |
| `documents.download` → `DownloadCaseDocument` | `pegasus.read`; current-user document/case policy, opaque server-selected custody ID only | `(true,false,true,true)` |
| `documents.export` → `ExportCaseDocuments` | `pegasus.write`; current-user document/case policy, same-case selected IDs and recorded export event | `(false,false,true,true)` |
| `requests.create_box` → `CreateBoxFileRequest`; `requests.revoke_box` → `RevokeBoxFileRequest`; `requests.create_upload` → `CreateRequestUploadLink`; `requests.revoke_upload` → `RevokeRequestUploadLink`; `vehicle.request_lookup` → `RequestVehicleLookup`; `vehicle.accept_suggestion` → `AcceptVehicleSuggestion`; `reports.generate_eva` → `GenerateEvaHandoff`; `reports.link_evidence` → `LinkReportEvidence`; `reports.unlink_evidence` → `UnlinkReportEvidence` | `pegasus.write`; current-user case policy plus accepted external-adapter/evidence gate | `(false,true,true,true)` |

Tool results are bounded projections, never server exceptions, tokens, secrets, custody coordinates or raw inbound HTML. `openWorld=true` identifies a possible external-system effect, not permission. Static tests expand every semicolon-separated row into individual expected entries and fail on an extra/missing name, duplicate name, mismatched Core contract, schema, policy, scope or hint. Caller tests invoke every entry through OAuth/HTTP, including direct denial after role/account/grant/client revocation.
...
| `/Administration`, `/Administration/Accounts`, `/Administration/Access`, `/Administration/Roles`, `/Administration/Organizations`, `/Administration/Principals`, `/Administration/Configuration`, `/Administration/Mailboxes` | Administrator-only landing plus exact account/access/role, dual-role organization, principal successor, configuration and approved-mailbox workspaces. No generic rule editor, OAuth-client administration UI, secret field or cloud operation. |
...
#### 6.2.1 Razor Page caller matrix

These are the required physical PageModel owners. Brace notation means each named `.cshtml` has a matching `.cshtml.cs`; no catch-all/admin-generic PageModel or direct DbContext mutation may replace a row.

| Route | Physical PageModel file(s) | GET query/transport | POST Core command(s) | Required browser proof beyond the global auth/error matrix |
|---|---|---|---|---|
| `/` | `Pages/Index.cshtml(.cs)` | `GetOperationsSnapshot` | none; refresh is GET | exact-filter navigation; zero/unavailable/stale/partial/failure are distinct. |
| `/Operations/Email` | `Pages/Operations/Email.cshtml(.cs)` | `GetEmailOperations` | `RetryMailboxProcessing` for an owned durable failure only | Received/Sent, bounded failure, retry/replay and deep link. |
| `/Operations/Requests` | `Pages/Operations/Requests.cshtml(.cs)` | `GetRequestOperations` | `RetryExternalWork`, `RevokeBoxFileRequest`, `RevokeRequestUploadLink` | active/expired/exhausted/revoked/failed/unknown-external outcomes. |
| `/Intake` | `Pages/Intake/Index.cshtml(.cs)` | `ListIntake` | `ReceiveIntake` for authenticated bounded manual upload | empty/filter/page/upload validation and PRG-preserved safe input. |
| `/Intake/{id:guid}` | `Pages/Intake/Details.cshtml(.cs)` | `GetIntake` | `ResolveIntake`, `ReevaluateIntake`, `AcceptIntake`, `LinkIntake`, `ReverseIntakeLink` | fact/suggestion/confirmed provenance; Needs sorting/Blocked intake/technical failure; accept/link idempotency. |
| `/Intake/{id:guid}/Source` | `Pages/Intake/Source.cshtml.cs` (handler only) | `DownloadIntakeSource` | none | current-user receipt authorization, safe content type/disposition, no inbound HTML execution, wrong-ID denial without existence disclosure. |
| `/Triage` | `Pages/Triage/Index.cshtml(.cs)` | `ListTriage` | none | empty/filter/page/state/optional-assignee. |
| `/Triage/{id:guid}` | `Pages/Triage/Details.cshtml(.cs)` | `GetTriage` | `AssignTriage`, `UnassignTriage`, `RecordTriageFinding`, `SupersedeTriageFinding`, `LinkTriageResponseEvidence`, `UnlinkTriageResponseEvidence`, `CompleteTriage`, `CancelTriage`, `ReopenTriage`, `LinkTriageCase`, `UnlinkTriageCase` | independent optional findings, exact reply evidence, correction/supersession, terminal/reopen and no due/chaser controls. |
| `/Cases` | `Pages/Cases/Index.cshtml(.cs)` | `SearchCases` | none | every named filter, URL preservation, no unauthorized-result count leakage. |
| `/Cases/{id}` | `Pages/Cases/Details.cshtml(.cs)` plus focused semantic partials under `Pages/Cases/Shared/` | `GetCase` | `ConfirmCompleteness`, `AcquireCaseEditLease`, `RenewCaseEditLease`, `ReleaseCaseEditLease`, `SaveCase`, `HoldCase`, `ReleaseCase`, `TransitionCase`, `CloseCase`, `ReopenCase`, `ArchiveCase`, `CreateLinkedReplacement`, `CreateCaseTask`, `AssignCaseTask`, `CompleteCaseTask`, `CancelCaseTask`, `RecordEngineerFinding`, `AddCaseDocument`, `LogicallyRemoveDocument`, `CreateBoxFileRequest`, `RevokeBoxFileRequest`, `CreateRequestUploadLink`, `RevokeRequestUploadLink`, `RequestVehicleLookup`, `AcceptVehicleSuggestion`, `GenerateEvaHandoff`, `LinkReportEvidence`, `UnlinkReportEvidence` | read-only without lease; every section populated from real projection; stale rowversion/lease loss/idempotent replay/external unknown; reasoned lifecycle and immutable history. |
| `/Cases/{caseId}/Documents/{documentId}/Download` | `Pages/Cases/Documents/Download.cshtml.cs` (handler only) | `DownloadCaseDocument` | none | current/historical version authorization, safe content type/disposition, hash and wrong-case/version/path denial without disclosure. |
| `/Cases/{caseId}/Documents/Export` | `Pages/Cases/Documents/Export.cshtml.cs` (handler only) | none | `ExportCaseDocuments` | selected same-case IDs only, deterministic ZIP/manifest/collision handling, mid-stream failure and no arbitrary custody ID. |
| `/Administration` | `Pages/Administration/Index.cshtml(.cs)` | static authorized child-route registry | none | only routes with implemented callers; direct non-admin denial. |
| `/Administration/Accounts` | `Pages/Administration/Accounts/{Index,Edit}.cshtml(.cs)` | `ListStaffAccounts`, `GetStaffAccount` | `CreateStaffAccount`, `DisableStaffAccount` | forced first change, disabled-session denial, no delete/password display. |
| `/Administration/Access` | `Pages/Administration/Access/Index.cshtml(.cs)` | `GetAccessReview` | `ReviewStaffAccess`, `RevokeStaffMcpAuthorizations` | due/overdue review, reason/history and token denial after revoke. |
| `/Administration/Roles` | `Pages/Administration/Roles/Index.cshtml(.cs)` | `GetRoleAssignments` | `AssignStaffRoles` | Administrator/Engineer/User combinations, last-admin protection and current-session re-evaluation. |
| `/Administration/Organizations` | `Pages/Administration/Organizations/{Index,Edit}.cshtml(.cs)` | `ListOrganizations`, `GetOrganization` | `CreateOrganization`, `UpdateOrganizationRoles` | independently selectable Work Provider/Instruction Intermediary roles, duplicate/empty-role/active-principal guards. |
| `/Administration/Principals` | `Pages/Administration/Principals/{Index,Create,Replace}.cshtml(.cs)` | `ListOrganizations`, `GetOrganization` | `CreatePrincipal`, `ReplacePrincipal` | immutable principal/reference ownership, disabled predecessor, linked successor and no in-place principal mutation. |
| `/Administration/Configuration` | `Pages/Administration/Configuration.cshtml(.cs)` | `GetWorkflowConfiguration` | `UpdateWorkflowConfiguration` | versioned completeness/review gates, stale-version denial and no secret/cloud fields. |
| `/Administration/Mailboxes` | `Pages/Administration/Mailboxes.cshtml(.cs)` | `ListApprovedMailboxes` | `UpdateApprovedMailbox` | approved-address/route scope only; no browsing, credential or send control. |
| `/Search` | `Pages/Search/Index.cshtml(.cs)` | `SearchCases` | none | exact query in URL, empty/no-match and authorization-safe result counts. |
| `/Account/SignIn`, `/Account/PasswordChange`, `/Account/SignOut`, `/Account/AccessDenied` | corresponding files under `Pages/Account/` | ASP.NET Core Identity transport only | sign-in/password-change/sign-out transport; Core account policy remains injected | generic failure, throttle, forced change, return-URL safety, CSRF and disabled/changed-role session. |
| `/Development/EmailEvaluation` | `Pages/Development/EmailEvaluation.cshtml(.cs)` | Development evaluator only | bounded evaluation command; no persistence | endpoint absent from production metadata and nav; genuine input capture remains disabled. |
| `/Uploads/{token}` | `Pages/Uploads/Request.cshtml(.cs)` | `GetRequestUpload` | `UploadToRequest` | no case/request identity disclosure; expiry/count/size/hash/idempotency and terminal generic errors. |
| OAuth metadata/authorize/token/revocation and `/mcp` | `Pages/Connect/Authorize.cshtml(.cs)` for human consent; transport endpoints in `Program.cs`; no MCP Razor Page | OpenIddict/MCP transport over the static manifest | approve/deny/revoke transport only; each tool then calls its mapped Core contract | metadata/resource/audience/PKCE/consent/token rotation/throttle/revocation and every manifest tool through HTTP. |

The implementation may split a large Case or Administration PageModel only by adding a named route and updating this table, capability map, tests and navigation in the same reviewed change. It may not collapse distinct workspaces into a client-side SPA or create a placeholder route whose Core caller arrives later.
...
- Initialisation runs root `dotnet tool restore`, `dotnet restore --locked-mode` and `npm ci`, installs pinned browsers, checks Development HTTPS/LocalDB/Functions/Azurite and creates ignored state only.
...
The optional Cloud profile pins the supported Azure CLI/azd/Bicep/GitHub/Box/SqlServer/Exchange tools, but neither login nor profile selection authorises a read/write. The sole cloud preflight owner is checkpoint 14's `scripts/Test-AzureDeploymentPlan.ps1 -Mode Target`, separately gated by exact subscription/resource-group/identity and operation approval; no undefined helper or profile selection may perform a read. Infisical is not an alpha runtime or deployment dependency.
...
- Use the current stable generated official .NET SDK only after exact enterprise, application/service user, root and operations are approved. The only Box folder ID authorized/in scope for creation of case files is 392761581105, accessed using the already authenticated Box CLI; do not imply broader Box access or perform any external operation. Store Box key material only in the owning environment's Key Vault and retrieve it through the owning managed identity.
...
- Prepare distinct Pegasus Development and Production targets in the existing `infra/` and `azure.yaml`; remove the complete Document Intelligence parameter/resource/output/role/config chain, the Infisical chain, `infra/scripts/Grant-DatabaseAccess.ps1`, and the `azure.yaml` `postprovision` grant hook rather than leaving dormant settings or broad grants. Per environment keep one Linux Web App (Development F1, Production B1), one Linux FC1 Functions app, one Azure SQL logical server/database (Development Basic, Production S0), two Standard LRS storage accounts: a Functions transport/deployment account containing the two ID-only work queues `intake-work`/`external-work`, their explicit Functions poison queues `intake-work-poison`/`external-work-poison`, host internals and the deployment-package container; and an application custody/protection account with separate private transient-intake, Web-authentication-ring and Box-link-ring containers, one Standard Key Vault, one Log Analytics/Application Insights pair with 31-day retention, and distinct user-assigned Web/Worker identities. Preserve the accepted alpha public-service-endpoint topology and Azure SQL `AllowAllWindowsAzureIps` reachability with Entra/RBAC enforcement; private networking remains `Not planned`, not a dormant migration.
- Keep the existing one Linux FC1, .NET-isolated Worker deployment unit and its required `functionAppConfig`/deployment storage; adding one app per trigger would violate the accepted boundary. Use source-verified supported Bicep API versions, disable shared-key access and public containers on both accounts, retain Key Vault soft-delete/purge protection, and never reuse `AzureWebJobsStorage` as an application-storage client. Grant the Worker `Storage Blob Data Owner` plus `Storage Table Data Contributor` on the transport account for the Functions host/diagnostics and `Storage Queue Data Contributor` only on the four named queues; grant Web only `Storage Queue Data Message Sender` on the two work queues. On the application account, grant Web `Storage Blob Data Contributor` only on the transient-intake, Web-authentication-ring and Box-link-ring containers, and Worker `Storage Blob Data Contributor` only on transient-intake and Box-link-ring. These are data-plane role assignments from the exact Bicep identity outputs; neither identity receives storage management/list-keys access, and Worker must be denied the Web-authentication container. `azure.yaml` names only Web/Worker services and sets Worker `remoteBuild: false`; target settings require the supported .NET 10 isolated stack, `FUNCTIONS_WORKER_RUNTIME=dotnet-isolated`, `AzureWebJobsStorage__accountName`, `AzureWebJobsStorage__credential=managedidentity`, `AzureWebJobsStorage__clientId=<Worker output>`, `functionAppConfig.deployment.storage.authentication.type=UserAssignedIdentity` with the exact Worker identity resource ID, distinct `ApplicationStorage__accountName`/`ApplicationStorage__clientId=<own identity output>` settings for SDK clients, and `Production` environments. Application Insights disables local authentication and uses its non-secret connection string plus `APPLICATIONINSIGHTS_AUTHENTICATION_STRING=ClientId=<own identity>;Authorization=AAD` and `Monitoring Metrics Publisher` for each host identity. Web uses its user-assigned identity and `ASPNETCORE_ENVIRONMENT=Production`. Neither target may contain `DevelopmentOffline`, development signing keys, storage account keys, SQL passwords/connection strings or `SCM_DO_BUILD_DURING_DEPLOYMENT=true`.
- Azure SQL is Entra-only. Before exact-target validation can bind an administrator object ID, the release owner uses a separately approved exact Entra write to create—or verify and adopt—one dedicated, assigned-membership, security-enabled, non-mail SQL administrator/migrator group; record its approved name, tenant and immutable object ID, prove it has no members and grant it no unrelated role. This identity prerequisite is not an application deployment. Bicep creates the Web/Worker user-assigned identities, sets that group as SQL Entra administrator and emits each identity's object/client IDs. Each host receives `ConnectionStrings__Pegasus=Server=tcp:<server>.database.windows.net,1433;Database=<database>;Authentication=Active Directory Managed Identity;User Id=<its Bicep-output client ID>;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30`; no name lookup, password or system-assigned fallback is permitted. The immutable schema bundle creates only reviewed least-privilege application roles/grants. During an approved migration window, the release principal receives temporary group membership, obtains a fresh token and applies the bundle. The committed bootstrap script then consumes the provisioned Bicep identity outputs, converts each SQL `UNIQUEIDENTIFIER` client ID to `VARBINARY(16)`, uses pinned `sqlcmd` with `ActiveDirectoryDefault` and strict TLS to create contained users by client-ID `SID`, `TYPE = E`, and assigns only the owning role. Remove membership, discard the token, wait for propagation/token expiry, acquire a new token and prove DDL denied before cutover; readiness uses read-only `HAS_PERMS_BY_NAME` denial and the deployed caller smoke proves each identity's allow/deny matrix.
...
- Define the release-input tree hash deterministically from tracked path, mode and file bytes for the reviewed repository except the sole outcome record `docs/changes/2026-07-27-qdos-alpha-reference-corpora.md`. Before path enumeration, prune the forbidden `docs/reference/imp-docs/` prefix and immutable `corpus/`; also exclude Git metadata, timestamps and ignored `artifacts/`. Emit excluded-prefix path-name counts only, never target stat/content/hash data. This deliberately makes any source, dependency, IaC, migration, script, runbook, product, design or other documentation change invalidate the release input. A post-deployment evidence commit may change only that one excluded outcome record, and the gate separately proves the exact base/descendant diff before accepting the unchanged release-input hash.
- Locally run restore/build/test, Bicep build/lint and deployment-plan consistency. Only when every implementation/local/replay prerequisite and exact target/identity/artifact-custody decision is complete may `.azure/deployment-plan.md` move from its current target-design-only status to `Ready for Validation`; exact subscription/resource-group read and validation approval then permits the committed policy/quota/region/RBAC/what-if checks, and only a fully passing recorded result moves it to `Validated`.
- Never invoke `azd up` or another resource mutation from the implementation PR. The centralized `docs/operations.md` release procedure, governed by ADR-0009, pins `azd`, target and environment; after merge and exact write approval it permits `azd provision --environment <approved-environment>`, the separate SQL bootstrap/migration boundary, then `azd deploy web --environment <approved-environment> --from-package <hash-verified-web.zip>`, Web readiness, and finally `azd deploy worker --environment <approved-environment> --from-package <hash-verified-worker.zip>` plus Worker smoke. It refuses `azd up`, release-time `azd package`, unscoped/plain `azd deploy`, remote build, a path/hash outside the immutable manifest or a target mismatch. The release owner obtains/revalidates approved artifact custody before these commands and proves health/rollback custody after them. The executing harness uses its required Azure workflow skills without making tool names repository authority; every target-dependent managed-identity/remote proof follows provisioning and blocks cutover, not source completion.
...
### 9.1 Capability activation and blocker matrix

A capability row is not an implementation license by itself. Before authoring its dependent production behavior, record the exact decision/evidence below in the existing change record; tests and source-only adapters may be prepared, but no dormant caller, route, schema, migration or deployment setting may pre-empt the decision. Missing evidence blocks the named IDs and therefore blocks `ImplementationCandidate`/release; it never silently removes an ID or substitutes fabricated data.

| Capability IDs | Required activation decision/evidence | Owning checkpoint and proof |
|---|---|---|
| `EVAL-01`–`EVAL-05`, `MAIL-20`–`MAIL-22` | Accepted local folder/evaluation contract: exact unchecked/checked roots, taxonomy identity/hash, reviewer identity/reasoning, move semantics, rule display and genuine-corpus capture restrictions. | 1, 5, 9; evaluator caller and untouched holdout. |
| `INT-03`, `INT-22`, automatic portions of `TRI-01`–`TRI-09` | Accepted direct-provider/intermediary route matrix for every activated QDOS principal: forwarding shape, authorised domains/senders, instruction/report/Triage predicates, ambiguity outcome and provider-policy version. | 1, 5, 7, 9; route-by-route evaluator and Graph replay. |
| `INT-17` | Mechanism-neutral candidate set, licence/security/platform/egress/cost terms, preaccepted accuracy/abstention/latency gate, frozen genuine cohort plus untouched holdout, and one accepted engine/release artifact. | 9; AI Centre evaluation plus actual Infrastructure/Web/Worker caller. |
| `INT-21` | Operator-reviewed extraction field cohort/holdout, exact per-field truth representation, conflicts/missing values, acceptance thresholds and immutable source/package hashes. | 1, 5, 9; field-level report and untouched holdout. |
| `INT-31` | Exact lifetime, aggregate/per-file byte and file-count limits, allowed content types, per-token/IP rate, one-time/reuse behavior, custody transition, retry/idempotency, revocation/expiry and safe error contract. | 1, 6, 10; anonymous HTTP browser tests and abuse/replay matrix. |
| `EXT-01`, `EXT-02` | Exact DVLA/DVSA product/licence/fields, credential owner, source-of-truth and mileage-estimation rule, egress/retention/rate/cost/failure/rotation contracts. | 1, 9, 13; deterministic replay then approved target caller. |
| `EXT-03` | Operator acceptance of the ordered 13-key UTF-8 JSON, null/empty/date/mileage normalization, image selection/order, custody-confirmation rule and SHA-256 manifest. | 1, 9, 10; byte-exact fixture and genuine case export, no EVA network call. |
| `EXT-18` | Accepted physical-address evidence precedence and exact `Image Based Assessment` instruction mapping; conflict/missing evidence remains unresolved. | 1, 4, 10; Core mapper and UI provenance. `DATA-02` is not consulted. |
| `MAIL-14`, `MAIL-16`, `TRI-05` and report/Triage automatic evidence | Exact approved-mailbox/Graph scope, immutable Sent-item ID/change tracking, reply/thread/reference/attachment predicates, mailbox owner and mismatch/recovery rules. | 1, 5, 7, 13; immutable replay then approved mailbox read. |
| `ACC-05`, `ACC-06`, `UI-11` | Organization/Principal model and routes in sections 4.1/6.2.1: dual independent Work Provider/Instruction Intermediary roles, immutable principal, disabled linked successor and sequence continuity. | 3, 4, 10; Core invariants plus every Administration route. |
| `UI-13` | Exact Edge Stable/Windows 11 versions, Narrator operator, keyboard/forced-colours/200%-zoom route matrix and evidence-capture policy. | every UI checkpoint and 12; pinned Chromium CI plus approved Edge/Narrator acceptance. |
| `DOC-01`–`DOC-08` | Exact Box enterprise/user/root/test subtree, folder template/file-request operations, credential owner/rotation, private Blob lifecycle and custody/rollback contract. | 1, 6, 8, 13; local replay, disposable approved subtree, then target identity denial/success. |
| `MCP-01`–`MCP-04` | Static manifest in section 5.9, approved public client/redirect metadata, current-user grant/role semantics and exact remote-client compatibility path. | 3, 6, 11, 13; local OAuth/HTTP all-tool matrix then approved remote client. |
| `OPS-01`–`OPS-25` target-dependent subset | Exact Azure target names/subscription/resource-group/resource disposition; SQL admin/migrator identities; alert/budget recipients/scope; release custody/retention; performance dataset/burst; restore owner; operator and management approvals. | 12–14 and release sequence; validation/live evidence only after exact approval. |

`DATA-02` is canonically `Next / 0.2.0`. No alpha code inspects its spreadsheets or adds an address-reference schema, migration, reader, UI, caller, package or fallback. All other future-target and Not planned capabilities remain absent except a seam that a Now aggregate demonstrably needs and that the capability inventory explicitly preserves.

### 9.2 Executable application acceptance gate
...
Release-boundary verification compares the exact 229-ID canonical inventory and exact target of every row with source, schema, migration, endpoints, PageModels, navigation, Worker metadata, MCP manifest, configuration and release contents. It must report all 128 alpha IDs owned and proved, zero future/Not planned activation, and no unknown ID; the matrix above defines the only decision-gated alpha subsets.

## 10. New-worktree and checkpoint-commit execution

### 10.1 Activation and worktree creation

No QDOS production-code command runs until both prerequisite heads exist and pass their own exact-head gates: (1) documentation centralization, then (2) capability versioning/Project synchronization as its child. Record both immutable SHAs, PR URLs/checks/reviews and their ancestry in issue #3 and the existing QDOS change record. The two prerequisite PRs may be open: acceptance here means this work uses their reviewed exact heads as an explicit stack, not that an agent merged them or claimed they were repository `main` authority.

Issue #3 remains the sole QDOS delivery identity. Do not open a plan-activation PR or create another QDOS change record. Checkpoint 1 first adds the clause-level ADR-0014 addendum and distills every normative decision, capability-blocker, Worker, MCP and Razor caller matrix from this accepted plan into the existing change record/canonical owners; production code starts only after that checkpoint commit is reviewed in the QDOS PR.

Create a clean unique short-path worktree from the exact capability-versioning head. If both prerequisite PRs remain unmerged, the QDOS PR targets the capability-versioning branch; if they are already merged in order, branch from that exact `main` merge lineage and target `main`. Never merge, reset, force-move, auto-rebase or copy prerequisite commits:

```powershell
git fetch origin
$DocumentationHeadSha = '<recorded exact documentation PR head>'
$CapabilityHeadSha = '<recorded exact capability PR head>'
git cat-file -e "$DocumentationHeadSha^{commit}"
git cat-file -e "$CapabilityHeadSha^{commit}"
git merge-base --is-ancestor $DocumentationHeadSha $CapabilityHeadSha
if ($LASTEXITCODE -ne 0) { throw 'Capability head is not a descendant of the documentation head.' }
$QdosBranch = 'workflow/20260728-deliver-qdos-alpha' # choose a unique recorded suffix if occupied
$QdosWorktree = 'C:\src\pegasus-alpha'             # choose a unique short path if occupied
git worktree add -b $QdosBranch $QdosWorktree $CapabilityHeadSha
Set-Location $QdosWorktree
```

Before editing, prove the base contains conflict-free `docs/capabilities.md` with the exact 229 rows/targets, centralized owner paths, the capability change record and no forbidden-prefix operation in planned proof. Read back Project 3 to prove all 229 keyed capability items exist exactly once: 128 `Now / In progress / 0.1.0-alpha.1` drafts reference issue #3, 72 future drafts remain `Triage` at their exact versions, and 29 boundary drafts remain `Not planned / unallocated` with no issue or milestone. Freeze the accepted QDOS contract hash and base SHA in its change record. Do not copy `.git`, `artifacts/`, `corpus/`, `temp-plan/`, local settings, package caches or credentials. Approved corpus material remains read-only and is referenced by hash from ignored run state only when its evidence checkpoint begins.

The QDOS PR and its exact base/head preserve this stack explicitly. After maintainers merge prerequisite PRs in order, they may retarget the unchanged descendant PR to `main` only after ancestry and diff review; any changed prerequisite head invalidates the QDOS base and requires a reviewed replacement branch/replay through the repository-permitted non-merge route plus the full exact-head gate.
...
- Add and review ADR-0014 with the exact clause-level supersession table in section 3.1; update `docs/decisions/README.md` and the existing QDOS change record with this plan's decisions, blocker matrix, Worker/MCP/Razor caller matrices, exact prerequisite heads and issue #3. Verify the centralized `TRI-04`, `UI-03`, `DOC-11`, `EXT-18`, state-token, browser, three-Now-outcome and design-target wording without silently reinterpreting it.
...
- Extend the centralized repository guard only where needed so the exact `docs/reference/imp-docs/` prefix is pruned before every enumeration/read/hash operation and canonical product/design files reject conflict markers; preserve its established no-argument CI behavior.
- Verify, do not regenerate unnecessarily, the immutable provider-domain source/package/hash/schema/migration/catalog and the exact 229-ID/128-alpha target matrix inherited from the capability prerequisite.
...
- Repository policy reports 229 unique capabilities, 128 at `0.1.0-alpha.1`, exact future targets for 72, 29 Not planned/unallocated, no marker, unknown or duplicate, and zero future-target caller/schema/UI activation.
...
- Issue #3, the sole QDOS change record, requirements, capability inventory, open decisions, ADR-0014 and design traceability agree on target/evidence states and exact prerequisite heads.

**Commit:** `docs: activate QDOS alpha execution contract`
...
- Add Microsoft.Playwright `1.61.0` and Deque.AxeCore.Playwright `4.12.0` only to the existing Integration test project. Its `Browser` trait must launch the package-pinned Chromium entirely offline, run axe against a deterministic readiness document, then render and scan the actual synthetic intake-review markup and served stylesheet returned by the Web HTTP caller. Missing browser/runtime, launch failure, caller failure or any automated axe violation fails the lane with rule identifiers only. This caller proof does not activate Edge Stable, Narrator, authenticated critical-journey, deployment or operator-acceptance claims; absent external/manual evidence remains a fail-closed release gate. Update only the centralized root quick start and `docs/operations.md` local-development/testing procedures—no replacement runbook tree.
- Commit root `.config/dotnet-tools.json` pinned to `dotnet-ef` `10.0.10`; enable lock-file restore for every `Pegasus.slnx` project and commit its `packages.lock.json`; preserve the root `package-lock.json`. Cut over `Directory.Build.props`, `package.json` and `package-lock.json` cleanly from `0.0.0-development` to `0.1.0-alpha.1`; expose that version plus source SHA only in the release manifest/non-sensitive diagnostics, and add one guard rejecting any project, assembly, npm lock, runtime or artifact-manifest mismatch. `azure.yaml` is orchestration rather than a product-version authority and must not gain an ignored duplicate version field.

**Exit proof**

- Fresh worktree setup succeeds; actual Web and Functions host answer readiness.
- The selected `Browser` trait proves the pinned Chromium/axe dependency path without a cloud/vendor call and scans the actual synthetic Web review caller. It fails rather than skips when its browser binary is absent; production Edge/Narrator and manual accessibility evidence remain unclaimed until separately executed and accepted.
...
- `dotnet tool restore`, `dotnet restore --locked-mode` and `npm ci` consume only committed manifests/locks. Assemblies, project/central properties, npm metadata/lock, diagnostics and artifact manifest all report `0.1.0-alpha.1`; the guard fails on each deliberately mismatched source and on an unlocked dependency change.
...
- Execute the genuine QDOS cohort/untouched-holdout campaign for the activated direct/intermediary routes, extraction, Triage and exact-report predicates. Do not revive the superseded 88/56-candidate general-provider campaigns; additional-provider routes retain their exact future targets and remain inactive in alpha. Record each accepted predicate separately; insufficient evidence remains fail closed.
...
- Add the OAuth-client registration/revocation/emergency-disable procedure and remote-client prerequisites to centralized `docs/operations.md` without activating AI-09.
...
- Update centralized `docs/operations.md` and the change-record evidence matrix with exact capability/caller/variant coverage, revision, command, result, artifact hash and limitation; create no replacement runbook tree.
...
6. Full restore/build/tests/repository check, exact-head CI and clean-workstation and operator `docs/operations.md` procedure exercises are green. Each publishable browser/axe bundle records source SHA, OS/browser, seed/profile, routes, result and hash and passes the no-business-content/token/file artifact guard; genuine-data runs record only hashes/counts/results with capture disabled.
...
**13a Graph/Exchange:** add only the Infrastructure Graph adapter, production configuration validator and replay/live-test profile. Contract proof exercises the four-mailbox allowlist, `instructions@…` Inbox ingestion, all four approved Sent-evidence folders, pre-call folder/action denial, delta/MIME, immutable IDs, `410` bounded resync, SDK throttling/retry and absence of write/send operations. Record the Exchange Application RBAC grant/revoke/emergency-disable owner and centralized `docs/operations.md` procedure. Permission for all four mailboxes, denial of a fifth control and managed-identity caller proof can occur only after the target Worker identity and policy exist. **Commit:** `feat(mail): add the accepted Graph mailbox adapter`.
...
- Require the checkpoint 2 `0.1.0-alpha.1` version to remain identical in `Directory.Build.props`, `package.json`, `package-lock.json`, every project/assembly, dependency lock, package manifest and non-sensitive diagnostic, with source SHA as separate informational metadata; prove `.config/dotnet-tools.json` still pins `dotnet-ef` `10.0.10` and do not restamp during packaging. Add `scripts/Build-ReleaseArtifacts.ps1`, `scripts/Invoke-AzureDatabaseBootstrap.ps1` and `scripts/Test-AzureDeploymentPlan.ps1`; update existing Bicep/parameters/`azure.yaml`. Delete `infra/scripts/Grant-DatabaseAccess.ps1` and remove the `azure.yaml` `postprovision` hook rather than adapting the broad name-based grant. Remove `documentIntelligenceEnabled`, the Document Intelligence account/output/role, Infisical runtime configuration and every unused role/setting from Bicep, application configuration and release manifests. Enforce exact user-assigned Web/Worker identities and Bicep-output-derived RBAC; identity-based Functions host storage (`AzureWebJobsStorage__accountName`, `AzureWebJobsStorage__credential=managedidentity` and `AzureWebJobsStorage__clientId` from the Worker-identity Bicep output, never an account key/connection string); separate Key Vault/Data Protection signing custody; both work queues and both poison queues; completed-only seven-day Blob lifecycle; 31-day telemetry retention plus accepted sampling/daily cap; readiness and the named operational alert matrix/Action Group; Entra-only SQL; explicit schema/principal bootstrap; `Production` host environments with no Development keys/adapters; `remoteBuild: false`; and immutable locally built package consumption. Add the subscription-scope £75 monthly budget filtered to both exact Pegasus resource groups with actual 80%/100% and forecast 100% notifications; pass explicit month-first start and supported end dates from reviewed release parameters rather than runtime `utcNow`. All generated packages/manifests/bootstrap SQL stay under ignored `artifacts/` or approved CI/release custody.
...
- `Test-AzureDeploymentPlan.ps1 -Mode Local` fails if `Grant-DatabaseAccess.ps1`, a post-provision grant hook, Document Intelligence/Infisical material, shared storage keys, SQL secrets, Development settings, `SCM_DO_BUILD_DURING_DEPLOYMENT`, remote build, a package path outside the immutable manifest, or a mismatched target/version survives. It proves each required Bicep output and target binding before target validation.
- The centralized `docs/operations.md` release procedure preserves migration → Web live/ready → Worker live/ready ordering and names exact artifact custody; temporary SQL administrator/migrator membership; client-ID SQL bootstrap; concealed, transactional Andrew/Alex/public-client `InitializeApplication`; permanent post-completion denial and ordinary Administrator recovery; dependency/alert/managed-identity/remote-client smoke; trigger-by-trigger enablement; rollback/restore evidence; and every exact approval.
...
pwsh ./scripts/Test-RepositoryLanguage.ps1
pwsh ./scripts/Invoke-Doctor.ps1 -Profile Offline
pwsh ./scripts/Initialize-LocalDevelopment.ps1
...
Update the smallest post-centralization owner in the same checkpoint that changes behavior:

- Intended product/allocation: `docs/requirements.md`, `docs/capabilities.md` and `docs/open-decisions.md`. Capabilities changes only for an accepted requirement/target/link correction—never implementation status.
- Change/evidence: the existing `docs/changes/2026-07-27-qdos-alpha-reference-corpora.md` is the sole QDOS plan/evidence/outcome record. This temporary execution plan is not a second repository status owner.
- Design: `design/README.md`, `design/product/requirements.md`, `design/product/ui-spec.md`, `design/product/traceability-matrix.md` and their retained source/runtime/token/foundation/component/pattern contracts; brand assets remain authoritative.
- Current architecture and procedures: `docs/architecture.md`, `docs/operations.md`, `.azure/deployment-plan.md` and the existing dated Azure inventory/replacement plan. Do not recreate deleted runbook, testing-index or status subtrees.
- Operator authority: `docs/operator-notes.md` changes only when the operator supplies or resolves business truth, never to make implementation conform retroactively. Repository workflow changes belong to `docs/engineering.md`; routing changes belong to `docs/index.md` and root `README.md`.
- Source: existing `src/Pegasus.Core`, `src/Pegasus.Infrastructure`, `src/Pegasus.Web`, `src/Pegasus.Worker`, tests/scripts/CI/infra, plus `workspaces/ai-centre` only for isolated VRM candidate/evaluation ownership; never add it to the solution, application references or deployment.

Do not create a roadmap/status/runbook replacement, rewrite operator truth to fit code, or add generated handoff/status JSON or another workflow database.

## 13. Post-PR release handoff required for true full-alpha status

Required maintainer merge order is strict: **PR 1 documentation centralization → PR 2 capability allocation/Project synchronization → PR 3 QDOS implementation**. Each child must retain or be revalidated against the exact accepted parent before retargeting; never merge PR 2 or PR 3 first. The later one-file release-evidence PR is **PR 4**, opened only after approved Development/Production deployment and acceptance; it is not part of the implementation stack and cannot retroactively repair PR 3.
...
Repository authority consumed after the two prerequisite plans complete:

- `docs/index.md`, `docs/requirements.md`, `docs/capabilities.md` and `docs/open-decisions.md`.
- Source-labelled authoritative operator truth in `docs/operator-notes.md`.
- `design/README.md`, `design/product/requirements.md`, `design/product/ui-spec.md`, `design/product/traceability-matrix.md`, and retained foundation/token/brand/component/pattern sources.
- Accepted ADRs/decisions, including ADR-0001–ADR-0009 where present, Decisions 0010–0013 and the reviewed clause-level ADR-0014 addendum; immutable earlier bodies remain evidence and their preserved remainders stay authoritative.
- `docs/architecture.md`, `docs/operations.md`, `docs/engineering.md`, current Core/Infrastructure/Web/Worker callers, tests, scripts, CI, IaC, deployment plan, dated authorized Azure inventory and the active change record. Deleted pre-centralization pages are represented only through the accepted disposition/provenance proof, not reopened as parallel authority.
...
- Azure Functions .NET isolated support, Queue triggers and identity-based host connections: <https://learn.microsoft.com/azure/azure-functions/dotnet-isolated-process-guide>, <https://learn.microsoft.com/azure/azure-functions/functions-bindings-storage-queue-trigger>, <https://learn.microsoft.com/azure/azure-functions/functions-reference>, <https://learn.microsoft.com/azure/azure-functions/manage-connections#define-connections>, <https://learn.microsoft.com/azure/azure-functions/functions-infrastructure-as-code> and <https://learn.microsoft.com/azure/azure-functions/flex-consumption-how-to#configure-deployment-settings>

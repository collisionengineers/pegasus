...
Full design token reference. All values are in `styles/colors_and_type.css` as CSS custom properties.
...
Brand face files are in `fonts/` (`TwCenMTStd*.otf`, `FuturaCyrillic*.ttf`). Declared in `styles/colors_and_type.css` with `@font-face` + `font-display: swap`. Use brand faces for **logo, display, marketing moments, printed reports** — not for long body copy.

...
{
    [TestMethod]
    public void Read_MailWithRecipientBodiesAndAttachment_ProjectsEvidence()
    {
        ...
        Assert.AreEqual(MsgItemKind.Mail, document.Projection.Kind);
        Assert.AreEqual("Synthetic subject", document.Projection.Fields["subject"]);
        Assert.HasCount(1, document.Recipients);
        Assert.AreEqual("To", document.Recipients[0].Role);
        Assert.AreEqual("Plain body", document.Bodies.CanonicalText);
        Assert.AreEqual("plain", document.Bodies.CanonicalSource);
        Assert.HasCount(1, document.Attachments);
        CollectionAssert.AreEqual(new byte[] { 1, 2, 3 }, document.Attachments[0].Content.ToArray());
    }

    [TestMethod]
    public void Read_FixedAndMultiValuedProperties_PreservesDecodedAndRawValues()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        Assert.AreEqual((short)-7, Find(document, 0x4001).Values[0].Decoded);
        Assert.IsTrue((bool)Find(document, 0x4002).Values[0].Decoded!);
        Assert.HasCount(3, Find(document, 0x4004).Values);
        Assert.AreEqual("two", Find(document, 0x4005).Values[1].Decoded);
        Assert.AreEqual(MapiValueKind.Raw, Find(document, 0x4006).Values[0].Kind);
        Assert.Contains("MSG_PROPERTY_TYPE_UNSUPPORTED", document.Issues.Select(static issue => issue.Code));
    }

    [TestMethod]
    public void Read_SingleGuid_UsesDedicatedValueStreamAndPreservesRawBytes()
    {
        var fixture = new MsgFixture();
        ...
        MsgDocument document = MsgReader.Read(fixture.Build());
        MapiValue value = Assert.ContainsSingle(Find(document, 0x4007).Values);

        Assert.AreEqual(MapiValueKind.Identifier, value.Kind);
        Assert.AreEqual(expected, value.Decoded);
        CollectionAssert.AreEqual(expected.ToByteArray(), value.RawBytes.ToArray());
        Assert.DoesNotContain("MSG_PROPERTY_VALUE_INLINE_INVALID", document.Issues.Select(static issue => issue.Code));
    }

    [TestMethod]
    public void Read_MultipleGuids_UsesContiguousFixedWidthValueStream()
    {
        var fixture = new MsgFixture();
        ...
        Assert.AreEqual(first, values[0].Decoded);
        Assert.AreEqual(second, values[1].Decoded);
    }

    [TestMethod]
    public void Read_SingleGuidWithoutValueStream_RetainsExplicitMissingEvidence()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        MsgDocument document = MsgReader.Read(fixture.Build());
        MapiValue value = Assert.ContainsSingle(Find(document, 0x4009).Values);

        Assert.AreEqual(MapiValueKind.Raw, value.Kind);
        Assert.IsEmpty(value.RawBytes);
        Assert.Contains("MSG_PROPERTY_VALUE_STREAM_MISSING", document.Issues.Select(static issue => issue.Code));
        Assert.AreEqual(MsgReadOutcome.Partial, document.Outcome);
    }

    [TestMethod]
    public void Read_NamedProperties_ResolvesNumericAndStringNames()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        Assert.IsNotNull(numericCandidate);
        NamedPropertyIdentity numeric = numericCandidate;
        Assert.AreEqual((uint)0x1234, numeric.NumericName);
        NamedPropertyIdentity? textualCandidate = Find(document, 0x8001).NamedIdentity;
        Assert.IsNotNull(textualCandidate);
        NamedPropertyIdentity textual = textualCandidate;
        Assert.AreEqual("SyntheticName", textual.StringName);
    }

    [TestMethod]
    public void Read_ExternalAndOleAttachments_RecordsPassiveIssuesWithoutRetrieval()
    {
        var fixture = new MsgFixture();
        ...
        Assert.AreEqual("https://example.invalid/passive", document.Attachments[0].PassiveReference);
        Assert.IsEmpty(document.Attachments[0].Content);
    }

    [TestMethod]
    public void Read_EmbeddedMessage_RecursesWithinLimit()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(Variable(0x001A, 0x001F, "IPM.Note"));
        ...
        Assert.IsNotNull(embeddedCandidate);
        MsgDocument embedded = embeddedCandidate;
        Assert.AreEqual(MsgItemKind.Contact, embedded.Projection.Kind);

        var limits = MsgReadLimits.Default with { MaximumNestingDepth = 0 };
        MsgDocument limited = MsgReader.Read(fixture.Build(), limits);
        Assert.AreEqual(MsgReadOutcome.ResourceLimitExceeded, limited.Outcome);
    }

    [TestMethod]
    [DataRow("IPM.Appointment", MsgItemKind.Calendar)]
    [DataRow("IPM.Schedule.Meeting.Request", MsgItemKind.Meeting)]
    [DataRow("IPM.Contact", MsgItemKind.Contact)]
    [DataRow("IPM.DistList", MsgItemKind.DistributionList)]
    [DataRow("IPM.Task", MsgItemKind.Task)]
    [DataRow("IPM.StickyNote", MsgItemKind.Note)]
    [DataRow("IPM.Activity", MsgItemKind.Generic)]
    [DataRow("REPORT.IPM.Note.NDR", MsgItemKind.Report)]
    public void Read_MessageClass_ProjectsDeclaredSemanticKind(string messageClass, MsgItemKind expected)
    {
        var fixture = new MsgFixture();
        ...
        Assert.AreEqual(expected, document.Projection.Kind);
    }

    [TestMethod]
    public void Read_ProtectedClass_ReturnsEncryptedWithoutDecryption()
    {
        var fixture = new MsgFixture();
        ...
        Assert.AreEqual(MsgReadOutcome.Encrypted, document.Outcome);
        Assert.Contains("MSG_PROTECTED_CONTENT", document.Issues.Select(static issue => issue.Code));
    }

    [TestMethod]
    public void Read_CancelledToken_ReturnsCancelled()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(Variable(0x001A, 0x001F, "IPM.Note"));
        ...
        Assert.AreEqual(MsgReadOutcome.Cancelled, document.Outcome);
    }

    [TestMethod]
    public void Read_String8CodePageDeclaredAfterValue_DecodesOnceWithoutFallback()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        Assert.AreEqual("€", document.Projection.Fields["subject"]);
        Assert.AreEqual(MsgReadOutcome.Complete, document.Outcome);
        Assert.DoesNotContain("MSG_CODEPAGE_FALLBACK", document.Issues.Select(static issue => issue.Code));
    }

    [TestMethod]
    public void Read_ConflictingCodePages_UsesFirstAndReportsConflict()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Fixed(0x3FFD, 0x0003, 1252),
        ...
        Assert.AreEqual("IPM.Note", document.Projection.MessageClass);
        Assert.Contains("MSG_CODEPAGE_CONFLICT", document.Issues.Select(static issue => issue.Code));
        Assert.HasCount(1, document.Issues.Where(static issue => issue.Code == "MSG_CODEPAGE_CONFLICT"));
    }

    [TestMethod]
    public void Read_VariableMultiValueZeroOneAndMany_UsesFourByteLengthRecords()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        Assert.HasCount(1, Find(document, 0x4100).Values);
        Assert.IsEmpty(Find(document, 0x4100).Values[0].RawBytes);
        Assert.HasCount(1, Find(document, 0x4101).Values);
        Assert.HasCount(2, Find(document, 0x4102).Values);
    }

    [TestMethod]
    public void Read_MalformedVariableMultiValue_RetainsActualAndTrailingRawBytes()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        Assert.HasCount(2, property.Values);
        Assert.AreEqual(MapiValueKind.Raw, property.Values[0].Kind);
        CollectionAssert.AreEqual(new byte[] { 9, 8 }, property.Values[0].RawBytes.ToArray());
        CollectionAssert.AreEqual(new byte[] { 0xAA }, property.Values[1].RawBytes.ToArray());
    ...
    [TestMethod]
    public void Read_PropertyLimit_PreservesEarlierEvidenceAndReturnsResourceOutcome()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        Assert.AreEqual(MsgReadOutcome.ResourceLimitExceeded, document.Outcome);
        Assert.AreEqual("retained", document.Projection.Fields["subject"]);
        Assert.HasCount(2, document.Properties);
    }

    [TestMethod]
    public void Read_PartiallyProjectedClassAndGenericClass_NeverReturnComplete()
    {
        ...
        Assert.AreEqual(MsgReadOutcome.Partial, reportDocument.Outcome);
        Assert.Contains("MSG_ITEM_CLASS_PARTIAL", reportDocument.Issues.Select(static issue => issue.Code));
        Assert.AreEqual(MsgReadOutcome.Partial, genericDocument.Outcome);
        Assert.Contains("MSG_CLASS_GENERIC", genericDocument.Issues.Select(static issue => issue.Code));
    }

    [TestMethod]
    public void Read_ProtectedAndResourceLimited_ResourceOutcomeTakesPrecedence()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
    ...
    [TestMethod]
    public void Read_OleCustomChildStorage_IsExposedOnlyAsPassiveEvidence()
    {
        var fixture = new MsgFixture();
        ...
        MsgAttachment attachment = Assert.ContainsSingle(document.Attachments);
        MsgPassiveStorage passive = Assert.ContainsSingle(attachment.PassiveStorages);
        Assert.AreEqual("CustomOleStorage", passive.StorageName);
        Assert.Contains("MSG_ATTACHMENT_CHILD_STORAGE_PASSIVE", document.Issues.Select(static issue => issue.Code));
    }

    [TestMethod]
    public void Read_EmbeddedGenericMessage_RetainsNestedLocalIssues()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(Variable(0x001A, 0x001F, "IPM.Note"));
        ...
        Assert.AreEqual(MsgReadOutcome.Partial, embedded.Outcome);
    ...
    [TestMethod]
    public void Read_TransportHeadersAndUnicodeHtmlBody_AreProjectedWithoutBinaryMisdecode()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        Assert.AreEqual("From: sender@example.invalid\r\n", document.Projection.Fields["transportHeaders"]);
        Assert.AreEqual("<p>Unicode Ω body</p>", document.Bodies.HtmlText);
        Assert.AreEqual("Unicode Ω body", document.Bodies.CanonicalText);
        Assert.IsEmpty(document.Bodies.HtmlBytes);
    }

    [TestMethod]
    public void Read_DivergentPlainAndHtmlBodies_RetainsBothAndReportsIssue()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        Assert.AreEqual("plain", document.Bodies.PlainText);
        Assert.AreEqual("<p>different</p>", document.Bodies.HtmlText);
        Assert.Contains("MSG_BODY_VARIANTS_DIVERGE", document.Issues.Select(static issue => issue.Code));
    }

    [TestMethod]
    public void Read_UnsignedSparseAndDuplicateRecipientSuffixes_HaveStableOrderingAndIssue()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(Variable(0x001A, 0x001F, "IPM.Note"));
        ...
        Assert.AreEqual((uint)2, document.Recipients[0].SourceOrder);
        Assert.AreEqual((uint)2, document.Recipients[1].SourceOrder);
        Assert.AreEqual(0x80000000u, document.Recipients[2].SourceOrder);
        Assert.Contains("MSG_RECIPIENT_STORAGE_DUPLICATE", document.Issues.Select(static issue => issue.Code));
    }

    [TestMethod]
    public void Read_ReservedPropertyFlags_AreReportedAndCannotBeComplete()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
    ...
    [TestMethod]
    public void Read_ZeroLengthMultiValueItem_ConsumesCumulativeValueBudget()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        Assert.AreEqual(MsgReadOutcome.ResourceLimitExceeded, document.Outcome);
        Assert.IsEmpty(Find(document, 0x4100).Values);
    }

    [TestMethod]
    public void Read_NestedAttachmentBudget_IsCumulativeAndPreservesOuterAttachment()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(Variable(0x001A, 0x001F, "IPM.Note"));
        ...
        Assert.AreEqual(MsgReadOutcome.ResourceLimitExceeded, document.Outcome);
        MsgAttachment outer = Assert.ContainsSingle(document.Attachments);
        Assert.IsNotNull(outer.EmbeddedMessage);
        Assert.IsEmpty(outer.EmbeddedMessage.Attachments);
    }

    [TestMethod]
    public void Read_DuplicateNamedPropertyMapping_ReportsAndKeepsFirstIdentity()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(
            Variable(0x001A, 0x001F, "IPM.Note"),
        ...
        Assert.AreEqual((uint)0x1234, Find(document, 0x8000).NamedIdentity!.NumericName);
        Assert.Contains("MSG_NAMED_PROPERTY_DUPLICATE", document.Issues.Select(static issue => issue.Code));
    }

    [TestMethod]
    public void Read_WrongRootPropertyHeaderLength_IsRejectedByExactContext()
    {
        var fixture = new MsgFixture();
        ...
        Assert.AreNotEqual(MsgReadOutcome.Complete, document.Outcome);
    }

    [TestMethod]
    public void Read_NonZeroReservedRootHeader_IsReported()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(Variable(0x001A, 0x001F, "IPM.Note"));
    ...
    [TestMethod]
    public void Read_UnsupportedAttachmentMethod_RetainsAttachmentAndReportsPartial()
    {
        var fixture = new MsgFixture();
        fixture.AddRootProperties(Variable(0x001A, 0x001F, "IPM.Note"));
        ...
        Assert.AreEqual(99, Assert.ContainsSingle(document.Attachments).Method);
    ...
    private static MapiProperty Find(MsgDocument document, ushort id) =>
        Assert.ContainsSingle(document.Properties.Where(property => property.PropertyId == id));

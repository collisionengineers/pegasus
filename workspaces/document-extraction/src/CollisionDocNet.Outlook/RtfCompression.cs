...
public static class RtfCompression
{
...
internal static class PassiveRtfText
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (int index = 0; index < rtf.Length; index++)
        {
            ...
            else if (!skip && word is "par" or "line") output.AppendLine();
            else if (!skip && word == "tab") output.Append('\t');
            else if (word == "uc" && hasNumber) unicodeFallback = Math.Clamp(number, 0, 16);
            else if (!skip && word == "u" && hasNumber)
            {

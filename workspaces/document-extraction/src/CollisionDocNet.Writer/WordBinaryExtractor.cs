...
internal static class WordBinaryExtractor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                // PlcPcd includes one separator CP between the main document
                // and the specialised document parts when any such part exists
                // (MS-DOC 2.8.35).
            ...
            WordStructuredEvidence structured = WordStructuredEvidenceParser.Extract(
                compoundFile, wordDocument, table, selectedName, fib, pieces, limits, cancellationToken);
            issues.AddRange(structured.Issues);
            ...
            {
                PropertyRuns = structured.PropertyRuns,
                Structures = structured.Structures,
                PassiveAssets = structured.Assets,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (storyIndex == 1 && HasSpecializedStory(fib))
            {
                globalStart = checked(globalStart + 1);
            }
            ...
            if (storyLength != 0)
            {
                DecodeStory(wordDocument, pieces, fib.CharacterSet, StoryOrder[storyIndex], globalStart, globalEnd, segments, issues, cancellationToken);
    ...
    private static bool HasSpecializedStory(WordFib fib) =>
        fib.StoryLengths[1] != 0 ||
        fib.StoryLengths[2] != 0 ||
        fib.StoryLengths[4] != 0 ||
        fib.StoryLengths[5] != 0 ||
        fib.StoryLengths[6] != 0 ||
        fib.StoryLengths[7] != 0;
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (WordPiece piece in pieces)
        {
            ...
            for (uint cp = cpStart; cp < cpEnd; cp++)
            {
                ...
                char value = DecodeCodeUnit(wordDocument, fileOffset, piece.IsUnicode, characterSet, out bool unsupportedCodePage);
                if (unsupportedCodePage && !codePageIssueReported)
                {
                    issues.Add(new("doc-codepage-unsupported", "A non-ASCII compressed character could not be decoded from the declared FIB character set.", fileOffset));
                ...
                FlushText(text, runCpStart, cp, storyStart, runFileStart, fileOffset - runFileStart, piece.Index, segments);
            ...
            FlushText(text, runCpStart, cpEnd, storyStart, runFileStart, finalFileEnd - runFileStart, piece.Index, segments);
        }
    }
        ...
        ImmutableArray<WordTextSegment>.Builder segments)
    {
        ...
        segments.Add(new(WordTextSegmentKind.Text, text.ToString(), cpStart, cpEnd,
            cpStart - storyStart, cpEnd - storyStart, fileStart, byteLength, pieceIndex));
        text.Clear();
    }

    private static char DecodeCodeUnit(ReadOnlySpan<byte> bytes, uint offset, bool unicode, ushort characterSet, out bool unsupportedCodePage)
    {
        ...
        {
            if (value >= 0x80 && characterSet != 0)
            {
                unsupportedCodePage = true;
                return '\ufffd';
            }
    ...
    private static bool HasValidLimits(WordBinaryExtractionLimits limits) =>
        ...
        limits.MaximumPropertyRuns > 0 && limits.MaximumSprmsPerRun > 0 &&
        limits.MaximumStructureRecords > 0 && limits.MaximumPassiveAssets > 0 &&
        limits.MaximumPassiveAssetBytes >= 0;

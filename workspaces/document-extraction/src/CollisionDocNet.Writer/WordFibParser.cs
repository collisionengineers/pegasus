...
internal static class WordFibParser
{
        ...
        out WordBinaryIssue? issue)
    {
        ...
        ushort characterSet = U16(wordDocument, 20);
        uint fcMin = U32(wordDocument, 24);
        uint fcMac = U32(wordDocument, 28);
        if (fcMin > fcMac || fcMac > wordDocument.Length)
        {
            issue = new("doc-fib-text-range", "The FIB declares a WordDocument text range outside the stream.", 24);
            return false;
        }

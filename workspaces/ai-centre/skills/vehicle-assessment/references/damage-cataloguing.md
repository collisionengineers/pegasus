...
- Evidence label: `visible evidence`, `reasonable inference`, or `requires further evidence`
  (see `photo-and-evidence-intake.md`)

## Side determination checklist

Side errors are high-impact because they make the assessment look careless. Before writing
the pack or any operations:
...
- If side remains genuinely uncertain, state the assumption in the pack and use neutral
...
| Wheel kerb damage | Alloy refurb extra per kerbed wheel; geometry check on meaningful impacts |
...
| Fluid leaks visible | Radiator / coolant / power steering — identify the fluid and system before movement or road test |
| Damaged underbody visible | Subframe / crossmember check at minimum |
| Distorted shut lines or panel gaps | Possible structural displacement — see `structural-and-alignment-evidence.md` |
...
On borderline cases, state the assumption and invite the engineer to override. For the full
judgement factors (safety role, material, method, economics, corrosion reinstatement), read
`repair-renew-decision-matrix.md`.
...
## Unroadworthy statement — the standard phrase bank

Where the damage makes the vehicle unroadworthy, say so in the pack's roadworthiness section —
and, where the Audatex/EVA output is selected, in the payload `notes` field — using CE's
standard SOP pattern: **"Please note the vehicle is unroadworthy due to / as …"**, completed by
...
Use directional labels throughout: LHF (left hand front), RHR (right hand rear), nearside,
offside. Don't use front/rear alone when side matters.

## Describing the vehicle

Always state in the pack:
1. Vehicle identified from: [plate / badge / VIN plate / instrument cluster / description]
2. VIN decoded as: [full make/model/derivative] — decoded facts come from `vehicle-history-check`,
   not assumption

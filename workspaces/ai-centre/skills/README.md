# Imported agent-skill source

These source skill packs and pack-validation tools were imported from
`collisionengineers/collisionsuite` path `active/collision-agent-skills` and
colocated under the AI Centre workspace. Their exact provenance and manifest are
recorded in [`../../README.md`](../../README.md).

The source `ce-cost-defence.skill` ZIP package is materialized as
`ce-cost-defence/`; the archive itself is not retained. Source `_dev/` trees are
consolidated under `dev-ref/<skill-name>/`, with generated Python caches
excluded. Visual-reference PDFs remain external corpus material and are not
tracked in this workspace.

They are non-caller review evidence, not Pegasus runtime product code and not
coding-agent configuration. Nothing in this directory is loaded, invoked,

...
How should Pegasus expose the classified-email workspace through Automation Actor tools that call the same Core queries/commands and preserve staff-equivalent authorization, leases, replay and attribution?
...
- Current repository state: Pegasus.Web already hosts a gated Automation MCP ingress whose tools wrap existing Core use cases; no mail tools exist. This ticket follows UI-10/Mail actions and must add thin adapters only after their Core contracts land.

...
| Path | Why |
|---|---|
| `docs/engineering.md` | Replace the merge-commit release rule with the agreed manual, non-force, exact-SHA fast-forward procedure and linear release invariant. |
...
| `tests/Pegasus.ArchitectureTests/MainBranchHistoryGuardTests.cs` | Replace merge-only test expectations with accepted fast-forward promotion and rejected non-fast-forward rewrite. Do not claim the post-push guard can distinguish an authorized direct fast-forward from an ordinary direct fast-forward. |
...
| `.github/workflows/ci.yml` | The guard is a post-push CI check on `main`, while PR checks run independently; it is detective only under the agreed no-protection constraint. The `changes` job is shared with path classification, so preserve its outputs and checkout history. |
...
| GitHub repository settings for `collisionengineers/pegasus` | There are no `main` protection rules or rulesets. This is intentional for this ticket; post-push CI can detect but cannot prevent an ordinary direct update. |
...
- GitHub-side protection and rulesets are intentionally out of scope on subscription grounds; CI remains detective after a `main` push rather than preventive.
- The first release under the new policy needs a one-time non-rewriting convergence of `dev` with the existing `main` release commit before the fast-forward promotion. Subsequent releases fetch, prove `origin/main` is an ancestor of `origin/dev`, push the reviewed SHA to `main` without `--force`, then confirm both remote heads equal that SHA.

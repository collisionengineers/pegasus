...
| Path | Why |
|---|---|
| `tests/Pegasus.ArchitectureTests/DependencyDirectionTests.cs` | Replace the obsolete direct `IInstructionExtractionPolicy` constructor expectation with the existing `InstructionExtractionPolicySelector` boundary, while retaining the assertions that implementations are Core-owned and not duplicated in Infrastructure. |
...
|---|---|
| `src/Pegasus.Core/Intake/ProcessIntake.cs` | The authoritative current constructor has `InstructionExtractionPolicySelector`, invokes it for the instruction role, and does not own a direct policy parameter. Read-only evidence; do not change it. |
| `src/Pegasus.Core/Intake/InstructionExtractionPolicySelector.cs` | The selector is the Core collection/ambiguity boundary over signed extraction policies; keep an assertion that protects this rather than restoring an obsolete single-policy shape. |
...
| `docs/engineering.md` | Governs truthful failures, scope discipline and the sole heavy-verifier rule. |
...
The two assertions fail against the current runtime shapes, so their corrections allow the existing architecture suite to continue detecting a regression in the Core selection boundary or the Worker timer's exact reconciliation composition. No production caller, dependency registration, build asset, API, documentation or generated artifact changes.

...
The complete authoritative file manifest is supplied in pegasus_pack/astra_output/v1_implementation_plans/registers/file-ownership.csv and .json. Exact file then deepest prefix wins; ties are defects. Stream A may edit only its assigned files. A authors all Foundation files and explicit contract-only pre-F exceptions; B/C domain behavior remains with B/C.

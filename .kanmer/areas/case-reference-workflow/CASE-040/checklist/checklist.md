...
- [x] Blocker 1: `docs/design/test-ui/pages/queues--empty.html` restored to `origin/dev`'s 0/0/0 nav-count/tab-count capture (`git checkout origin/dev -- <path>`); the diff against `origin/dev` is now empty. CASE-040 renders nothing on the `/Cases` queues route, so the file carries no owned change and is dropped from this diff entirely.
- [x] Should-fix 2: `EvaSubmissionEntities.cs`'s `ExternalRef` comment reworded to name `EvaSubmissionPolicy.RequireOnceOnlyAutomaticSubmission` and the durable `ExternalWorkItems` row as the once-only guard, and to state the database now deliberately permits an explicit manual re-send — matching the already-rewritten sibling comments in `EvaSubmissionModelConfiguration.cs`.
- [x] Should-fix 3: added a direct store-level assertion to `AutomaticEvaSubmissionCompletesAfterDeliveredVersionConflictWithoutRetrying` — a wholly independent `EvaSubmissionStore.ExecuteAsync` automatic call over the now-delivered case throws `EvaAutomaticSubmissionAlreadyDeliveredException` with no further transport call, proving the once-only guard directly at the store level rather than only through the queued-work retry/exception-mapping path.
- [x] Nit 4: left for the release task per the reviewer's own disposition (deferred, no action) — `docs/current-architecture.md` is the as-built/deployed snapshot and CASE-040 is not yet deployed.
- [x] Verified locally: `dotnet restore --locked-mode` exit 0; `dotnet build --configuration Release --no-restore` exit 0, 0 warnings/errors; `Pegasus.Core.Tests` 1252 passed; `Pegasus.ArchitectureTests` 100 passed; `Pegasus.IntegrationTests` filtered to `CustodyOutboxIntegrationTests` — 23 passed, 1 pre-existing skip (includes the extended test above). No migration changed, so `Test-MigrationGrants.ps1` was not re-run; no routed Razor page, partial, or `catalogue.json` changed this round (the reverted file is a snapshot correction, not a page change), so no snapshot capture procedure applies.
- [x] Fixes committed (`41a92a1ed`) and pushed to `task/case-040-sign-off-engineer-eva`.

...
- [x] Snapshot evidence refreshed for head `f3005ea66` (supersedes the 41,040-byte line above): `case-details--default.html` 64,427 bytes, doctype, one `case-sticky`, eleven `id="section-<key>"` hosts, zero `<img src="#">`; `index.html` 12,562 bytes; regenerated by the scoped capture during the `origin/dev` merge, verified with `-Verify -SkipCapture` and `Test-UiCatalogue.ps1` (exit 0).
- [x] Round-3 record corrections applied to the post-implementation report (fragment URL, CASE-041 contract, Deviation 3).

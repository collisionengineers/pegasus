...
is a byte-identical two-file relocation plus the one necessary index target
repair. No application caller loads the package and no runtime route changes.
...
JSON regeneration command before the root-owned verifier slot. The approved

...
## F-001 complete-roster mutation preflight

The same readiness class also includes a known later associated/stale member: checking it only during the link loop can mutate an earlier ready member first. Before any new link in src/Pegasus.Web/Presentation/UploadCaseDecision.cs, inspect the complete server-owned roster, require every receipt, prove completed members form the identical decision's committed prefix, validate pending receipt versions and viable destinations against the original Case version plus proven completed prefix. Keep the existing per-link fresh concurrency/lease checks and honest partial failures that arise during writes. No new transaction, abstraction, contract or persistence. Extend tests/Pegasus.IntegrationTests/UploadConfirmationWebTests.cs with one data-driven real HTTP class proving a later different-target or same-target-different-operation association, stale version or missing reviewed member does not change the earlier ready receipt/Case/history. Existing interrupted success and replay evidence stays required. Controller approved this as F-001, not unrelated scope.

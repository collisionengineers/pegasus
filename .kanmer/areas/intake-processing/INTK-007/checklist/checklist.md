...
- [x] Test migration from clean and representative legacy databases.
...
Took over by operator decision. Merged `origin/dev` (42 commits, one conflict in `docs/capabilities.md`, resolved). Fixed the three blockers and all 14 remaining reviewer comments (dispositions in `plan.md`'s dated "Review fixes" section). Ran the final stale-term search and recorded the full classification in `post-implementation-report.md`. Renamed `MailOperationalDestination.NeedsSorting` to `Unidentified` (item 6 hand-off; TICK-045 had not merged into `origin/dev` — see plan.md and scratch-takeover.md). Found and fixed a real regression the branch's own commit introduced (three integration tests still asserted the literal rendered text "Needs sorting" after the label changed to "Unidentified"), and a real bug in this session's own reconciliation code (`ActionActor.SystemWorker` rejected by `UnidentifiedValidation.ValidateResolve`, which requires Staff or Automation — the reconciliation feature would have silently never worked; caught by writing `UnidentifiedPersistenceTests.cs`, the persistence-level test file that didn't exist before this session).

Corrected the runtime-role grants twice: first pass granted an identical matrix to both roles; the coordinator asked for per-object least privilege, so re-derived every permission from the actual caller (see plan.md's "Grant correction" note). Also added the `20260819115323_UnidentifiedWork` permission census to `scripts/Invoke-AzureDatabaseBootstrap.ps1` per a mid-session coordinator addition, kept in exact sync with the migration's own GRANT/DENY statements.

**Final verification, all against this branch's actual head:**
- `dotnet build ./Pegasus.slnx -c Release`: green throughout.
- Local copies of the not-yet-merged `Test-MigrationGrants.ps1` and `Test-AzureDeploymentPlan.ps1 -Mode Local` both pass for this ticket's own migration (the only remaining `-Mode Local` failure names `20260819104953_MailClassificationCorrectionHistory.cs`, confirmed not this ticket's — tracked by PR #426).
- `pwsh ./scripts/Test-DocumentationLinks.ps1`: all 204 files' relative Markdown links resolve.
- **Full `dotnet test ./Pegasus.slnx -c Release` (all four test projects, one combined run):** `Pegasus.Core.Tests` 655/655 passed; a 97/97-passed suite (Architecture); `Pegasus.IntegrationTests` 576 total, 561 passed, 1 failed, 14 skipped. The one failure, `QdosAllocationRecoveryTests.DistinctParallelRetriesResolveToOneCaseAggregate`, is a SQL Server deadlock in a pre-existing concurrency-stress test for `IntakeAllocation.cs`/`EfIntakeAllocationStore.cs` (untouched by this ticket) — confirmed flaky, not a regression, by re-running it alone afterward: it passed. `tests/Pegasus.IntegrationTests/UnidentifiedPersistenceTests.cs` (added after the full run started, so not in that count) — 3/3 passed on its own run against the real migration.
- Core test count grew from 592 (prior agent) to 655 this session (63 new/changed assertions across the retryable-failure, reason-mapping, and enum-rename fixes); IntegrationTests grew by 3 new persistence tests plus the corrected regression assertions.

Still explicitly unchecked and honestly outstanding: grouped-submission/INTK-005 integration (blocked on that ticket merging — the Core origin contract already accepts a SubmissionGroup id, but no caller produces one yet), retained-mail/Operations/search projection completion beyond the dashboard metric already wired, all-six-reason-mapping dedicated test coverage (one new case added — `ConflictingIdentification`; the other five reasons are exercised indirectly through existing decision-path tests but have no dedicated one-per-reason test), and a dedicated concurrent-allocation stress test (the existing `QdosAllocationRecoveryTests` concurrency pattern for Case allocation was not duplicated for Unidentified registration).

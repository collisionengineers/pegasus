...
updated: '2026-08-21T20:10:24.265Z'
---

## What

Recognise which third-party engineering firm issued the original report that
arrives with a QDOS **audit** instruction, and select the extraction method for
that document from what the issuer is known to produce — rather than running one
...
- Survey the local corpus for audit-instruction emails, take the non-instruction
  document attachment as the original report, and label each by issuing firm —
  from the report's own content (letterhead, footer, issuer block), never from
  the file name.
...
- Reuse the existing extraction policy route
  (`QdosInstructionExtractionPolicy` / `InstructionFieldExtraction`) — this adds
  issuer selection ahead of the grammar, it does not become a second extractor.
...
- [ ] Corpus survey recorded in the ticket's research: audit instructions found,
      issuers identified, count per issuer, and which reports could not be
      attributed.

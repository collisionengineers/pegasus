...
2. Add or extend the smallest Core contract/policy required to expose reasoned manual link/unlink/relink through the existing association transaction; validate identity, actor, reason, state and version before any write.
...
4. Wire the real caller (exact-message detail Case search/confirmation flow) through the Core use case with no duplicated taxonomy, mapping or authorization logic.
5. Add focused Core and integration/Web tests for authorization, reason, before/after history, stale versions, relink and Case navigation return context.
...
MAIL-01 identity; serial with TICK-051 on shared store.

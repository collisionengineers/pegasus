...
materialise one versioned mapping result containing separate operational destination and designated folder recommendation. Reuse `a dedicated Core mapping policy beside classification contracts`, keep Web/MCP callers thin, and place persistence or external mechanics only in `project/derive through retained-mail store without duplicating the table`. This follows the repository's one-Core-owner rule and the existing convention rather than adding a workspace-specific policy copy.
...
2. Add or extend the smallest Core contract/policy required to materialise one versioned mapping result containing separate operational destination and designated folder recommendation; validate identity, actor, reason, state and version before any write.
3. Implement the Infrastructure projection/transaction/adapter in project/derive through retained-mail store without duplicating the table; preserve mailbox scope, idempotency, optimistic concurrency and append-only evidence.
4. Wire the real caller (MAIL-05, UI-14 and MCP callers) through the Core use case with no duplicated taxonomy, mapping or authorization logic.
5. Add focused Core and integration/Web tests for exhaustive table, version identity, unsafe outcomes and no arbitrary destination.
...
operator mapping/folder answers and TICK-044.

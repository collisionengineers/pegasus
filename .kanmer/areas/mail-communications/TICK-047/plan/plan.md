...
return the current policy-designated Outlook folder recommendation for one classified message. Reuse `MAIL-23 mapping policy`, keep Web/MCP callers thin, and place persistence or external mechanics only in `approved-mailbox folder identity lookup`. This follows the repository's one-Core-owner rule and the existing convention rather than adding a workspace-specific policy copy.
...
2. Add or extend the smallest Core contract/policy required to return the current policy-designated Outlook folder recommendation for one classified message; validate identity, actor, reason, state and version before any write.
3. Implement the Infrastructure projection/transaction/adapter in approved-mailbox folder identity lookup; preserve mailbox scope, idempotency, optimistic concurrency and append-only evidence.
...
5. Add focused Core and integration/Web tests for confirmed mappings, no recommendation for unsafe outcomes, stale classification and arbitrary destination rejection.
...
operator folder mapping and TICK-064.

...
- [x] **What live Outlook/cloud verification is possible for the shared cross-mailbox policy?** — Resolved with the operator on 2026-08-19: production currently has only one linked mailbox, so TICK-045 must not require or claim a two-mailbox live check. Prove identical behavior for at least two mailbox identities with local/integration fixtures, and perform any read-only production verification only against the currently linked mailbox. Live evidence for another mailbox belongs to [[TICK-036]], [[TICK-037]], or [[TICK-038]] when that mailbox is actually connected.

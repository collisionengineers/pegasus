...
public sealed partial class AssessmentEstimateImportWebTests
{
    ...
    public async Task AnImportedEstimateIsRetainedAndLandsAsADraftWithProvenance()
    {
        ...
        // version; the estimate is the second half of one action and re-enters on the operator's
    ...
    /// <summary>
    /// ENG-026: Use estimate is the Engineer's acceptance — no typed-basis
    /// form; Core derives the calculation basis from the estimate's own
    /// totals and confirms a citing Draft-ready job.
    /// </summary>
    [Fact]
    public async Task UseEstimateRecordsTheEngineersAcceptance()
    {
        ...
        // The acceptance claims no lease of its own.
        Assert.Empty(store.LeaseClaims);

        var afterHtml = await GetHtmlAsync(client, $"/Cases/{caseId:D}/Assessment?estimate={draft.SpecificationId:D}");
        Assert.Contains("The estimate is now the case's current estimate.", afterHtml, StringComparison.Ordinal);
    }

    /// <summary>
    /// The editor's Save creates a named estimate from the typed header and
    /// lines through ISaveEstimate (ENG-026); an empty trailing row is
    ...
    public async Task TheEditorSavesANamedEstimateWithTypedLines()
    {
        ...
        // The editor posts repeated line-field keys, so this form keeps
        // duplicates instead of collapsing them into a dictionary.
        using var response = await client.PostAsync(
        ...
        var afterHtml = await GetHtmlAsync(client, $"/Cases/{caseId:D}/Assessment?estimate={store.LastCreatedEstimateId:D}");
        Assert.Contains("The estimate was saved.", afterHtml, StringComparison.Ordinal);
    }
    ...
    /// <summary>
    /// One recording fake for the substituted seams, so the tests can
    ...
    private sealed class RecordingStores(Guid caseId)
        : IGetCase, IGetAssessmentWorkspace, IRepairSpecificationStore, IAddCaseDocument,
          IAcquireCaseEditLease, ISaveEstimate, ISetCurrentEstimate
    {
        ...
        // The named-estimate path (ENG-026): the page goes through the
        // ISaveEstimate / ISetCurrentEstimate use cases below, so the raw
        // store seam stays unrouted here.

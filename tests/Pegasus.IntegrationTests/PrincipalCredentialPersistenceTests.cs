...
public sealed class PrincipalCredentialPersistenceTests
{
    ...
    public async Task IssueResetPauseResumeRevokeAreHashOnlyReplaySafeAndFailClosed()
    {
        ...
        var principal = await services.GetRequiredService<ICreatePrincipal>().ExecuteAsync(
            new(organization.Id, "QDOS", Administrator, "credential:principal:qdos"),
            default);
        ...
        Assert.Null(await get.ExecuteAsync(Administrator, principal.Id, default));

        // Issue: the secret comes back once; the replay of the same operation
        // key returns the record and no secret.
        var issueRequest = Request(principal.Id, 0, "credential:issue:1", "first key");
        ...
        Assert.Equal(principal.Id, authenticated.PrincipalId);
        ...
        var reset = await issue.ExecuteAsync(Request(principal.Id, 1, "credential:issue:2", "rotate"), default);
        ...
        var paused = await pause.ExecuteAsync(Request(principal.Id, 2, "credential:pause:1", "provider on hold"), default);
        ...
        Assert.Equal(
            ...
            (await Assert.ThrowsAsync<PrincipalCredentialException>(
                () => resume.ExecuteAsync(Request(principal.Id, 2, "credential:resume:stale", "stale"), default))).Error);
        var resumed = await resume.ExecuteAsync(Request(principal.Id, 3, "credential:resume:1", "provider back"), default);
        ...
        var revoked = await revoke.ExecuteAsync(Request(principal.Id, 4, "credential:revoke:1", "compromised"), default);
        ...
        Assert.Equal(
            ...
            (await Assert.ThrowsAsync<PrincipalCredentialException>(
                () => pause.ExecuteAsync(Request(principal.Id, 5, "credential:pause:revoked", "no"), default))).Error);
        var reissued = await issue.ExecuteAsync(Request(principal.Id, 5, "credential:issue:3", "new key"), default);
        ...
        var status = await get.ExecuteAsync(Administrator, principal.Id, default);

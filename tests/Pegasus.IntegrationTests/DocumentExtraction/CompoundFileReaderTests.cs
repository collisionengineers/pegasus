...
{
    [Fact]
    public void ReadRootDirectoryEntryIsRedReturnsValidatedDirectory()
    {
    ...
    [Fact]
    public void ReadRootDirectoryEntryHasUnknownColourReturnsInvalidDirectoryEntry()
    {
        byte[] bytes = CompoundFileFixture.CreateEmpty(3);
    ...
    [Fact]
    public void ReadChildTreeRootIsRedReturnsInvalidDirectoryTree()
    {
    ...
    [Theory]
    [InlineData((ushort)3, 512)]
    [InlineData((ushort)4, 4096)]
    public void ReadEmptyVersion3Or4FileReturnsValidatedDirectory(
        ushort version,
        int sectorSize)
    {
        ...
        Assert.True(result.IsSuccess);
        Assert.Equal(CompoundFileReadError.None, result.Error);
        Assert.NotNull(result.File);
        Assert.Equal(version, result.File.Header.MajorVersion);
        Assert.Equal(sectorSize, result.File.Header.SectorSize);
        Assert.Equal("Root Entry", result.File.DirectoryEntries[0].Name);
        Assert.Equal(CompoundFileObjectType.RootStorage, result.File.DirectoryEntries[0].ObjectType);
    }

    [Theory]
    [InlineData((ushort)3)]
    [InlineData((ushort)4)]
    public void ReadRegularStreamReturnsExactContent(ushort version)
    {
        ...
        Assert.Equal("Regular", stream.Name);
        Assert.Equal((ulong)4096, stream.StreamSize);
        Assert.Equal((uint)0, stream.ParentStreamId);
        Assert.Equal(4096, stream.Content.Length);
        Assert.Equal((byte)0xA7, stream.Content[0]);
        Assert.Equal((byte)0xA7, stream.Content[^1]);
    }

    [Fact]
    public void ReadMiniStreamReturnsExactContent()
    {
        ...
        Assert.True(result.IsSuccess);
        Assert.NotNull(result.File);
        Assert.Equal(expected, result.File.DirectoryEntries[1].Content.ToArray());
        Assert.Equal(128, result.File.MiniFat.Length);
    }

    [Fact]
    public void ReadDifatSectorReturnsAllFatSectorIds()
    {
        ...
        Assert.True(result.IsSuccess);
        Assert.NotNull(result.File);
        Assert.Equal(110, result.File.FatSectorIds.Length);
        Assert.Equal((uint)0, result.File.FatSectorIds[0]);
        Assert.Equal((uint)109, result.File.FatSectorIds[^1]);
    }

    [Fact]
    public void ReadDifatCycleReturnsInvalidDifat()
    {
        byte[] bytes = CompoundFileFixture.CreateWithDifat();
    ...
    [Fact]
    public void ReadCancelledTokenReturnsCancelledWithoutThrowing()
    {
        byte[] bytes = CompoundFileFixture.CreateEmpty(3);
        ...
        Assert.Equal(CompoundFileReadError.Cancelled, result.Error);
    }

    [Fact]
    public void ReadInputAboveConfiguredLimitReturnsInputLimitExceeded()
    {
        byte[] bytes = CompoundFileFixture.CreateEmpty(3);
    ...
    [Fact]
    public void ReadFatCycleReturnsFatCycle()
    {
        byte[] bytes = CompoundFileFixture.CreateWithRegularStream(3);
    ...
    [Fact]
    public void ReadTwoStreamsSharingARegularSectorReturnsCrossLink()
    {
        byte[] bytes = CompoundFileFixture.CreateWithRegularStream(3);
    ...
    [Fact]
    public void ReadDirectorySiblingCycleReturnsDirectoryTreeCycle()
    {
        byte[] bytes = CompoundFileFixture.CreateWithRegularStream(3);
        Span<byte> first = CompoundFileFixture.GetDirectoryEntry(bytes, 512, 1);
    ...
    [Fact]
    public void ReadStreamChainShorterThanDeclaredSizeReturnsLengthMismatch()
    {
        byte[] bytes = CompoundFileFixture.CreateWithRegularStream(3);
        Span<byte> first = CompoundFileFixture.GetDirectoryEntry(bytes, 512, 1);
    ...
    [Fact]
    public void ReadMiniStreamCrossLinkReturnsCrossLink()
    {
    ...
    [Fact]
    public void ReadMiniFatCycleReturnsMiniFatCycle()
    {
        byte[] bytes = CompoundFileFixture.CreateWithMiniStream([1, 2, 3]);
        Span<byte> root = CompoundFileFixture.GetDirectoryEntry(bytes, 512, 0);
    ...
    [Fact]
    public void ReadRightSiblingWithLowerSortKeyReturnsInvalidDirectoryTree()
    {
        byte[] bytes = CompoundFileFixture.CreateWithRegularStream(3);
        Span<byte> second = CompoundFileFixture.GetDirectoryEntry(bytes, 512, 2);
    ...
    [Fact]
    public void ReadUnallocatedDirectoryEntryWithDataReturnsInvalidDirectoryEntry()
    {
        byte[] bytes = CompoundFileFixture.CreateWithRegularStream(3);
    ...
    [Fact]
    public void ReadUnreachableAllocatedDirectoryEntryReturnsInvalidDirectoryTree()
    {
        byte[] bytes = CompoundFileFixture.CreateWithRegularStream(3);
        Span<byte> orphan = CompoundFileFixture.GetDirectoryEntry(bytes, 512, 2);
    ...
    [Fact]
    public void ReadDirectoryTreeWithUnequalBlackHeightButValidMsCfbRulesReturnsDirectory()
    {
        byte[] bytes = CompoundFileFixture.CreateWithRegularStream(3);
        Span<byte> first = CompoundFileFixture.GetDirectoryEntry(bytes, 512, 1);
        Span<byte> second = CompoundFileFixture.GetDirectoryEntry(bytes, 512, 2);
        ...
        Assert.True(result.IsSuccess);
        Assert.NotNull(result.File);
        Assert.Equal(3, result.File.DirectoryEntries.Where(static entry =>
            entry.ObjectType != CompoundFileObjectType.Unallocated).Count());
        Assert.Equal("Regular", result.File.DirectoryEntries[1].Name);
        Assert.Equal("Regular2", result.File.DirectoryEntries[2].Name);
    }

    [Fact]
    public void ReadValidInputDoesNotModifyCallerOwnedBytes()
    {
        byte[] bytes = CompoundFileFixture.CreateWithMiniStream([1, 2, 3]);

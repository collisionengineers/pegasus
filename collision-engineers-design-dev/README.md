...
- **Client-supplied fonts:** the **Tw Cen MT Std** and **Futura** families (in `fonts/`). The bundle is trimmed to the weights actually wired in `styles/colors_and_type.css` (Tw Cen Light/Regular/Italic/SemiBold/Bold/ExtraBold; Futura Book/Medium/Demi/Bold).
...
(A4, system-sans body, master-logo letterhead, red-rule section headings, red-bordered data tables).

**The canonical print spec — page setup, header/footer, titles, tables, value box, signatures, the
local Chromium render path and brand-font embedding — lives in
[`collision-engineers-design/references/document-letterhead.md`](collision-engineers-design/references/document-letterhead.md).**
Read it before building any document template; the composable components live in `ui_kits/documents/`.
(Kept here as a pointer only, to stop the spec drifting across copies.)
...
| `styles/colors_and_type.css` | All design tokens: `@font-face`, colour vars, type scale, radii, shadows, spacing. |
| `fonts/` | Brand typefaces — Tw Cen MT Std (6 OTF weights) + Futura Cyrillic (4 TTF weights). |
...
**Getting started:** link `styles/colors_and_type.css`, pull the master logo from `assets/`, load Lucide from CDN, and build with the tokens. For document templates, read `references/document-letterhead.md` first. **For any written copy, use the `ce-house-style` skill.**
...
- **Font variants:** `fonts/` carries only the weights declared in `styles/colors_and_type.css` — six Tw Cen MT Std OTF (Light, Regular, Italic, SemiBold, Bold, ExtraBold) and four Futura Cyrillic TTF (Book, Medium, Demi, Bold). Unwired condensed/ultra/italic variants were removed to keep the local bundle lean; add a weight back here and in the CSS if a future component needs it.
